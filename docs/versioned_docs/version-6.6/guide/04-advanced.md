...
The code is getting a bit messy, let's use [`em.create()`](/api/6.6/core/class/EntityManager#create) instead to make it clean again:
...
While we do not have such a use case in this guide, a huge benefit of using the [`EntityManager`](/api/6.6/core/class/EntityManager) with Unit of Work approach is automatic batching - all the `INSERT`, `UPDATE` and `DELETE` queries will be batched automatically into a single query per entity.
...
MikroORM has a simple [result caching](../caching.md) mechanism, all you need to do is add `cache` option to your [`em.find()`](/api/6.6/core/class/EntityManager#find) options. The value can be one of:
