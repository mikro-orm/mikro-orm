...
The `isInitialized()` method can be used for runtime checks, but that could end up being quite tedious - we can do better! Instead of manual checks for entity state, we can use the [`Reference`](/api/6.6/core/class/Reference) wrapper.
...
You can overcome this issue by using the [`Reference`](/api/6.6/core/class/Reference) wrapper. It simply wraps the entity, defining `load(): Promise<T>` method that will first lazy load the association if not already available. You can also use `unwrap(): T` method to access the underlying entity without loading it.
...
The `Loaded` type will represent what relations of the entity are populated, and will add a special `$` symbol to them, allowing for type-safe synchronous access to the loaded properties. This works great in combination with the [`Reference`](/api/6.6/core/class/Reference) wrapper:
...
When you define the property as [`Reference`](/api/6.6/core/class/Reference) wrapper, you will need to assign the [`Reference`](/api/6.6/core/class/Reference) instance to it instead of the entity. You can convert any entity to a [`Reference`](/api/6.6/core/class/Reference) wrapper via `ref(entity)`, or use the `wrapped` option of [`em.getReference()`](/api/6.6/core/class/EntityManager#getReference):
...
Since v5 we can also create entity references without access to [`EntityManager`](/api/6.6/core/class/EntityManager). This can be handy if you want to create a reference from inside the entity constructor:
...
If the reference already exist, you need to re-assign it with a new [`Reference`](/api/6.6/core/class/Reference) instance - they hold identity just like entities, so you need to replace them:
...
`Ref` is an intersection type that adds primary key property to the [`Reference`](/api/6.6/core/class/Reference) interface. It allows getting the primary key from [`Reference`](/api/6.6/core/class/Reference) instance directly.
