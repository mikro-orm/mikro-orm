...
const features = [
  ...
  },
  {
    ...
    description: 'Powerful event system with entity lifecycle hooks, onFlush events, seeding factories, and metadata-aware QueryBuilder.',
  },
];
...
const entityTabs = [
  {
    id: 'defineEntity',
    label: 'defineEntity',
    code: `import { defineEntity, p } from '@mikro-orm/core';
...
export class Book {
  ...
  },
  {
    id: 'entitySchema',
    label: 'EntitySchema',
    code: `export const BookSchema = new EntitySchema({
  name: 'Book',
  properties: {
...
const queryCode = `// fully type-safe queries
const books = await em.find(Book, {
  author: { name: 'Tolkien' },
}, {
  populate: ['author'], // type-checked!
  orderBy: { title: 'asc' },
});

// auto-completed, validated at compile time
books[0].author.name; // string, not undefined

// create & persist
const book = em.create(Book, {
  title: 'The Hobbit', author,
});
await em.flush(); // batched & transactional`;
