...
> `@CreateRequestContext()` decorator was added in v4.1.0

> Since v5, `@CreateRequestContext()` decorator is available in the `@mikro-orm/core`
...
Keep in mind, that all handlers that are decorated with @CreateRequestContext(), should NOT return anything.
