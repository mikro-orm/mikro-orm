...
| `ignorePolicies: boolean`                                 | Leave [row level security](./row-level-security.md) policies unmanaged. Defaults to `false`. When set to `true`, declared policies are still created and RLS is still enabled/forced based on the entity metadata, but existing policies are never dropped or altered and RLS is never disabled or unforced — use this to protect hand-written policies that are not mirrored in your entity definitions. |
