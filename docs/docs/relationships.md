...
To define a polymorphic relation, pass an array of entity types to the `@ManyToOne()` decorator and specify the `discriminator` option:
...
export const UserLike = defineEntity({
  ...
  properties: {
    ...
    likeable: () => p.manyToOne([Post, Comment], { discriminator: 'likeable' }),
  },
});
...
export class UserLike {
  ...
  // Polymorphic relation - can point to either Post or Comment
  @ManyToOne(() => [Post, Comment], { discriminator: 'likeable' })
...
export const UserLike = new EntitySchema<IUserLike>({
  ...
  properties: {
    id: { type: 'number', primary: true },
    // Polymorphic relation - can point to either Post or Comment
    likeable: { kind: 'm:1', entity: () => [Post, Comment], discriminator: 'likeable' },
  },
});
```
