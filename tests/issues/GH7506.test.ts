...
// GH #7506 — user `@Property({ persist: false })` class getters live on the
// entity prototype and were invoked by deep-clone helpers (e.g. `@vitest/utils`
// `clone`, `safe-stable-stringify`) walking the object graph via
// `Object.getOwnPropertyNames` + plain `val[key]`. The getter then ran with
// `this === prototype`, dereferenced unhydrated instance fields, and threw.
...
test('user getter on entity prototype is safe to read with `this === prototype` (GH7506)', async () => {
