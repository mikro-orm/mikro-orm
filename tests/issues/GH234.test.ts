...
describe('GH issue 234', () => {
  ...
  test('search by m:n (select-in)', async () => {
    ...
    expect(mock.mock.calls[1][0]).toMatch('select `b0`.`id`, `b0`.`a_id`, `b0`.`b_id`, `a1`.`id` as `a1__id`, `a1`.`name` as `a1__name` from `b_a_collection` as `b0` inner join `a` as `a1` on `b0`.`a_id` = `a1`.`id` where `b0`.`b_id` in (?) and `b0`.`a_id` in (?, ?, ?) order by `b0`.`id` asc');
    ...
    expect(mock.mock.calls[1][0]).toMatch('select `b0`.`id`, `b0`.`b_id`, `b0`.`a_id`, `b1`.`id` as `b1__id`, `b1`.`name` as `b1__name` from `b_a_collection` as `b0` inner join `b` as `b1` on `b0`.`b_id` = `b1`.`id` where `b0`.`a_id` in (?, ?, ?) and `b0`.`b_id` in (?, ?, ?) order by `b0`.`id` asc');
  ...
  test('search by m:n (joined)', async () => {
    const mock = mockLogger(orm, ['query']);
    const res1 = await orm.em.find(B, { aCollection: [1, 2, 3] }, { populate: ['aCollection'], strategy: 'joined', populateWhere: PopulateHint.INFER });
    ...
    orm.em.clear();
    mock.mock.calls.length = 0;
    const res2 = await orm.em.find(A, { bCollection: [1, 2, 3] }, { populate: ['bCollection'], strategy: 'joined', populateWhere: PopulateHint.INFER });
    expect(mock).toHaveBeenCalledTimes(1);
    ...
    expect(mock).toHaveBeenCalledTimes(1);
    expect(mock.mock.calls[0][0]).toMatch('select `b0`.*, `a1`.`id` as `a1__id`, `a1`.`name` as `a1__name` from `b` as `b0` left join `b_a_collection` as `b2` on `b0`.`id` = `b2`.`b_id` left join `a` as `a1` on `b2`.`a_id` = `a1`.`id` left join `b_a_collection` as `b3` on `b0`.`id` = `b3`.`b_id` where `b3`.`a_id` in (?, ?, ?) order by `b2`.`id` asc');
    expect(res3.map(b => b.id)).toEqual([1]);
  ...
  test('search by m:n (balanced)', async () => {
    const mock = mockLogger(orm, ['query']);
    const res1 = await orm.em.find(B, { aCollection: [1, 2, 3] }, { populate: ['aCollection'], strategy: 'balanced', populateWhere: PopulateHint.INFER });
    expect(mock).toHaveBeenCalledTimes(2);
    expect(mock.mock.calls[0][0]).toMatch('select `b0`.* from `b` as `b0` left join `b_a_collection` as `b1` on `b0`.`id` = `b1`.`b_id` where `b1`.`a_id` in (?, ?, ?)');
    expect(mock.mock.calls[1][0]).toMatch('select `b0`.`id`, `b0`.`a_id`, `b0`.`b_id`, `a1`.`id` as `a1__id`, `a1`.`name` as `a1__name` from `b_a_collection` as `b0` inner join `a` as `a1` on `b0`.`a_id` = `a1`.`id` where `b0`.`b_id` in (?) and `b0`.`a_id` in (?, ?, ?) order by `b0`.`id` asc');
    expect(res1.map(b => b.id)).toEqual([1]);
    ...
    orm.em.clear();
    mock.mock.calls.length = 0;
    const res2 = await orm.em.find(A, { bCollection: [1, 2, 3] }, { populate: ['bCollection'], strategy: 'balanced', populateWhere: PopulateHint.INFER });
    expect(mock).toHaveBeenCalledTimes(2);
    expect(mock.mock.calls[0][0]).toMatch('select `a0`.* from `a` as `a0` left join `b_a_collection` as `b1` on `a0`.`id` = `b1`.`a_id` where `b1`.`b_id` in (?, ?, ?)');
    expect(mock.mock.calls[1][0]).toMatch('select `b0`.`id`, `b0`.`b_id`, `b0`.`a_id`, `b1`.`id` as `b1__id`, `b1`.`name` as `b1__name` from `b_a_collection` as `b0` inner join `b` as `b1` on `b0`.`b_id` = `b1`.`id` where `b0`.`a_id` in (?, ?, ?) and `b0`.`b_id` in (?, ?, ?) order by `b0`.`id` asc');
    expect(res2.map(a => a.id)).toEqual([1, 2, 3]);
