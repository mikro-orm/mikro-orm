...
import { OracleDriver, OracleConnection } from '@mikro-orm/oracle';
import { initORMOracle, wipeDatabaseOracle } from './bootstrap';

import { Author, Book, BookTag, FooBar, FooBaz, Publisher, PublisherType, Test } from './entities';
...
describe('EntityManagerOracle', () => {
  ...
  beforeAll(async () => orm = await initORMOracle());
  beforeEach(async () => wipeDatabaseOracle(orm.em));
  ...
  test('getConnectionOptions()', async () => {
    const config = new Configuration({
      type: 'oracle',
      clientUrl: 'oracle://127.0.0.1:1521/db_name',
      host: '127.0.0.1',
      password: 'secret',
      user: 'user',
      logger: jest.fn(),
  ...
  test('should return oracle driver', async () => {
    const driver = orm.em.getDriver();
    expect(driver).toBeInstanceOf(OracleDriver);
    await expect(driver.findOne<Book>(Book.name, { title: 'bar' })).resolves.toBeNull();
    const author = await driver.nativeInsert(Author.name, { name: 'author', email: 'email' });
    const tag = await driver.nativeInsert(BookTag.name, { name: 'tag name' });
    expect((await driver.nativeInsert(Book.name, { uuid: v4(), author: author.insertId, tags: [tag.insertId] })).insertId).not.toBeNull();
    ...
    await expect(driver.getConnection().execute('insert into test (name) values (?)', ['test'], 'run')).resolves.toEqual({
      affectedRows: 1,
      insertId: 1,
    });
    await expect(driver.getConnection().execute('update test set name = ? where name = ?', ['test 2', 'test'], 'run')).resolves.toEqual({
      affectedRows: 1,
      insertId: 0,
    });
    await expect(driver.getConnection().execute('delete from test where name = ?', ['test 2'], 'run')).resolves.toEqual({
      affectedRows: 1,
      insertId: 0,
    });
    ...
    expect(driver.getPlatform().denormalizePrimaryKey('1')).toBe('1');
    await expect(driver.find<BookTag>(BookTag.name, { books: { $in: ['1'] } })).resolves.not.toBeNull();
    await expect(driver.ensureIndexes()).rejects.toThrowError('OracleDriver does not use ensureIndexes');
    ...
    await conn.transactional(async tx => {
      ...
      await conn.execute(orm.em.getRepository(Author).getKnex().raw('select 1'), [], 'all', tx);
    });
    ...
    // oracle returns the first inserted id
  ...
  test('connection returns correct URL', async () => {
    const conn1 = new OracleConnection(new Configuration({
      type: 'oracle',
      clientUrl: 'oracle://example.host.com',
      port: 1234,
      user: 'usr',
      password: 'pw',
    } as any, false));
    await expect(conn1.getClientUrl()).toBe('oracle://usr:*****@example.host.com:1234');
    const conn2 = new OracleConnection(new Configuration({ type: 'oracle', port: 3307 } as any, false));
    await expect(conn2.getClientUrl()).toBe('oracle://root@127.0.0.1:3307');
  });
  ...
  test('should allow to find by array of PKs', async () => {
    await orm.em.getDriver().nativeInsertMany(Author.name, [
  ...
  test('factory should support a primary key value of 0', async () => {
    ...
    const p2 = factory.create(Publisher, { id: 0 }); // shouldn't call constructor
  ...
  test(`1:1 relationships with an inverse side primary key of 0 should link`, async () => {
    ...
    const fooBazRef = orm.em.getReference<FooBaz>(FooBaz, 0);
    const fooBar = FooBar.create('testBar');
  ...
  test('inverse side of 1:1 is ignored in change set', async () => {
    const bar = FooBar.create('fb');
    bar.baz = new FooBaz('fz 1');
    await orm.em.persistAndFlush(bar);

    bar.baz = new FooBaz('fz 2');
    await orm.em.flush();
  });
  ...
  test('nested transactions with save-points', async () => {
    await orm.em.transactional(async em => {
      const god1 = new Author('God1', 'hello1@heaven.god');
      ...
      await em.transactional(async em2 => {
        const god2 = new Author('God2', 'hello2@heaven.god');
        em2.persist(god2);
      });
  ...
  test('nested transaction rollback with save-points will commit the outer one', async () => {
    ...
    const transaction = orm.em.transactional(async em => {
      ...
      try {
        await em.transactional(async em2 => {
          await em2.persistAndFlush(new Author('God', 'hello@heaven.god'));
          throw new Error(); // rollback the transaction
        });
      } catch { }

      await em.persist(new Author('God Persisted!', 'hello-persisted@heaven.god'));
    });
    ...
    expect(mock.mock.calls[2][0]).toMatch('insert into `author` (`created_at`, `email`, `name`, `terms_accepted`, `updated_at`) values (?, ?, ?, ?, ?)');
    expect(mock.mock.calls[3][0]).toMatch('rollback to savepoint trx');
    expect(mock.mock.calls[4][0]).toMatch('insert into `author` (`created_at`, `email`, `name`, `terms_accepted`, `updated_at`) values (?, ?, ?, ?, ?)');
  ...
  test('should load entities', async () => {
    ...
    const book = new Book('My Life on The Wall, part 2', author);
    book.createdAt = new Date(Date.now() + 2);
    ...
    const count2 = await orm.em.count(Author);
  ...
  test('json properties', async () => {
    ...
    const g = (await orm.em.findOne(Author, god.id, ['books']))!;
  ...
  test('findOne supports regexps', async () => {
    ...
    const author = new Author('Author 2', 'a2@example.com');
    const author3 = new Author('Author 3', 'a3@example.com');
    await orm.em.persistAndFlush([author1, author, author3]);
    orm.em.clear();

    const authors = await orm.em.find(Author, { email: /exa.*le\.c.m$/ });
    ...
    const authors2 = await orm.em.find(Author, { email: { $re: 'exa.*le.c.m$' } });
  ...
  test('findOne supports optimistic locking [versioned entity]', async () => {
    ...
    const test2 = await orm.em.findOne(Test, test.id, { lockMode: LockMode.OPTIMISTIC, lockVersion: test.version });
    await orm.em.lock(test2!, LockMode.OPTIMISTIC, test.version);
    const test3 = await orm.em.findOne(Test, test.id, { lockMode: LockMode.OPTIMISTIC, lockVersion: test.version });
  ...
  test('findOne supports optimistic locking [unversioned entity]', async () => {
    ...
    await expect(orm.em.lock(author, LockMode.OPTIMISTIC)).rejects.toThrowError('Cannot obtain optimistic lock on unversioned entity Author');
    await expect(orm.em.findOne(Author, author.id, { lockMode: LockMode.OPTIMISTIC })).rejects.toThrowError('Cannot obtain optimistic lock on unversioned entity Author');
  });
  ...
  test('lock supports optimistic locking [testLockUnmanagedEntityThrowsException]', async () => {
    const test = new Test();
    test.name = 'test';
    await expect(orm.em.lock(test, LockMode.OPTIMISTIC)).rejects.toThrowError('Entity Test is not managed. An entity is managed if its fetched from the database or registered as new through EntityManager.persist()');
  });

  test('pessimistic locking requires active transaction', async () => {
    const test = Test.create('Lock test');
    await orm.em.persistAndFlush(test);
    await expect(orm.em.findOne(Test, test.id, { lockMode: LockMode.PESSIMISTIC_READ })).rejects.toThrowError('An open transaction is required for this operation');
    await expect(orm.em.findOne(Test, test.id, { lockMode: LockMode.PESSIMISTIC_WRITE })).rejects.toThrowError('An open transaction is required for this operation');
  ...
  test('lock supports pessimistic locking [pessimistic write]', async () => {
    ...
    expect(mock.mock.calls[0][0]).toMatch('begin');
    expect(mock.mock.calls[1][0]).toMatch('select 1 from `author` as `e0` where `e0`.`id` = ? for update');
  ...
  test('lock supports pessimistic locking [pessimistic read]', async () => {
    ...
    expect(mock.mock.calls.length).toBe(3);
    expect(mock.mock.calls[0][0]).toMatch('begin');
    expect(mock.mock.calls[1][0]).toMatch('select 1 from `author` as `e0` where `e0`.`id` = ? lock in share mode');
    expect(mock.mock.calls[2][0]).toMatch('commit');
  });
  ...
  test('custom query expressions via query builder', async () => {
    ...
    const qb1 = orm.em.createQueryBuilder(Book);
    ...
    const qb2 = orm.em.createQueryBuilder(Book);
    ...
    const res3 = (await orm.em.findOne(Book, { 'JSON_CONTAINS(meta, ?)': [{ items: 1 }, true] } as any))!;
    expect(res3).toBeInstanceOf(Book);
  ...
  test('query builder getResult() and getSingleResult() return entities', async () => {
    ...
    expect(res1).toBeInstanceOf(Book);
    expect(res1!.createdAt).toBeDefined();
  ...
  test('stable results of serialization', async () => {
    ...
    const books0 = await orm.em.find(Book, []);
  ...
  test('stable results of serialization (collection)', async () => {
    ...
    for (const book of publisher.books) {
      expect(json.find((b: Book) => b.uuid === book.uuid)).toMatchObject({
        author: book.author.id,
      });
    }
  });
  ...
  test('populate OneToOne relation', async () => {
    ...
    const baz = new FooBaz('baz');
    bar.baz = baz;
    await orm.em.persistAndFlush(bar);
    orm.em.clear();

    const b1 = (await orm.em.findOne(FooBar, { id: bar.id }, ['baz']))!;
    expect(b1.baz).toBeInstanceOf(FooBaz);
  ...
  test('populate OneToOne relation on inverse side', async () => {
    ...
    const b0 = (await orm.em.findOne(FooBaz, { id: baz.id }))!;
    ...
    expect(b0.bar).toBeInstanceOf(FooBar);
    expect(wrap(b0.bar).isInitialized()).toBe(false);
    orm.em.clear();

    const b1 = (await orm.em.findOne(FooBaz, { id: baz.id }, ['bar']))!;
    ...
    expect(b2.bar).toBeInstanceOf(FooBar);
  ...
  test('populate OneToOne relation with uuid PK', async () => {
    const author = new Author('name', 'email');
    const book = new Book('b1', author);
    const test = Test.create('t');
    test.book = book;
    await orm.em.persistAndFlush(test);
    orm.em.clear();

    const b1 = (await orm.em.findOne(Book, { test: test.id }, ['test.config']))!;
  ...
  test('populate passes nested where and orderBy', async () => {
    ...
    const b4 = new Book('b4', author);
    const b5 = new Book('b5', author);
    ...
    const a1 = await orm.em.find(Author, author, ['books'], { books: { title: QueryOrder.DESC } });
    expect(a1[0].books.getItems().map(b => b.title)).toEqual(['b5', 'b4', 'b3', 'b2', 'b1']);
    orm.em.clear();

    const a2 = await orm.em.findOneOrFail(Author, author, ['books'], { books: { title: QueryOrder.DESC } });
    expect(a2.books.getItems().map(b => b.title)).toEqual(['b5', 'b4', 'b3', 'b2', 'b1']);
    orm.em.clear();

    const a3 = await orm.em.findOneOrFail(Author, { books: { tags: { name: { $in: ['silly', 'strange'] } } } }, ['books.tags'], { books: { tags: { name: QueryOrder.DESC }, title: QueryOrder.ASC } });
  ...
  test('many to many relation', async () => {
    ...
    expect(tags[0].books.isDirty()).toBe(false);
    expect(() => tags[0].books.getItems()).toThrowError(/Collection<Book> of entity BookTag\[\d+] not initialized/);
    expect(() => tags[0].books.remove(book1, book)).toThrowError(/Collection<Book> of entity BookTag\[\d+] not initialized/);
    expect(() => tags[0].books.removeAll()).toThrowError(/Collection<Book> of entity BookTag\[\d+] not initialized/);
    expect(() => tags[0].books.contains(book1)).toThrowError(/Collection<Book> of entity BookTag\[\d+] not initialized/);
    ...
    // test M:N lazy load
    orm.em.clear();
    let book = (await orm.em.findOne(Book, book1.uuid))!;
    ...
    orm.em.clear();
    book = (await orm.em.findOne(Book, book.uuid, ['tags']))!;
    ...
    book.tags.add(new BookTag('fresh'));
    await orm.em.persistAndFlush(book);
    orm.em.clear();
    book = (await orm.em.findOne(Book, book.uuid, ['tags']))!;
  ...
  test('many to many working with inverse side', async () => {
    ...
    const tag5 = new BookTag('sexy');
    book1.tags.add(tag1, tag3);
    book.tags.add(tag1, tag2, tag5);
    book3.tags.add(tag2, tag4, tag5);

    orm.em.persist([book1, book, book3, book4]);
    await orm.em.flush();
    orm.em.clear();

    let tag = await orm.em.findOneOrFail(BookTag, tag1.id, ['books']);
    expect(tag.books.count()).toBe(2);
    tag.books.add(orm.em.getReference(Book, book4.uuid));
    await orm.em.flush();
    orm.em.clear();
    ...
    tag = await orm.em.findOneOrFail(BookTag, tag1.id, ['books']);
    book4 = await orm.em.findOneOrFail(Book, book4.uuid, ['tags']);
    tag.books.add(book4);
  ...
  test('populating many to many relation on inverse side', async () => {
    ...
    book.tags.add(tag1, tag2, tag5);
    book3.tags.add(tag2, tag4, tag5);
    await orm.em.persistAndFlush([book1, book, book3]);
  ...
  test('nested populating', async () => {
    ...
    // as we order by Book.createdAt when populating collection, we need to make sure values will be sequential
    book1.createdAt = new Date(Date.now() + 1);
    book1.publisher = wrap(new Publisher('B1 publisher')).toReference();
    book1.publisher.unwrap().tests.add(Test.create('t11'), Test.create('t12'));
    book.createdAt = new Date(Date.now() + 2);
    book.publisher = wrap(new Publisher('B2 publisher')).toReference();
    book.publisher.unwrap().tests.add(Test.create('t21'), Test.create('t22'));
    book3.createdAt = new Date(Date.now() + 3);
    book3.publisher = wrap(new Publisher('B3 publisher')).toReference();
    book3.publisher.unwrap().tests.add(Test.create('t31'), Test.create('t32'));
    ...
    orm.em.clear();
    const books = await orm.em.find(Book, {}, ['publisher.tests', 'author'], { title: QueryOrder.ASC });
  ...
  test('hooks', async () => {
    expect(AuthorSubscriber.log).toEqual([]);
    Author.beforeDestroyCalled = 0;
    Author.afterDestroyCalled = 0;
    const repo = orm.em.getRepository(Author);
    const author2 = new Author('Jon Snow', 'snow@wall.st');
    expect(author2.id).toBeUndefined();
    expect(author2.version).toBeUndefined();
    expect(author2.versionAsString).toBeUndefined();
    expect(author2.hookParams).toHaveLength(0);

    await repo.persistAndFlush(author2);
    expect(author2.id).toBeDefined();
    expect(author2.version).toBe(1);
    expect(author2.versionAsString).toBe('v1');
    expect(author2.hookParams[0].em).toBe(orm.em);
    expect(author2.hookParams[0].changeSet).toMatchObject({ entity: author2, type: 'create', payload: { name: 'Jon Snow' } });

    author2.name = 'John Snow';
    await repo.persistAndFlush(author2);
    expect(author2.version).toBe(2);
    expect(author2.versionAsString).toBe('v2');
    expect(author2.hookParams[2].em).toBe(orm.em);
    expect(author2.hookParams[2].changeSet).toMatchObject({ entity: author2, type: 'update', payload: { name: 'John Snow' }, originalEntity: { name: 'Jon Snow' } });
    ...
    expect(AuthorSubscriber.log.map(l => [l[0], l[1].entity.constructor.name])).toEqual([
      ['beforeCreate', 'Author'],
      ['afterCreate', 'Author'],
      ['beforeUpdate', 'Author'],
      ['afterUpdate', 'Author'],
  ...
  test('subscribers', async () => {
    ...
    expect(EverythingSubscriber.log.map(l => [l[0], l[1].entity.constructor.name])).toEqual([
      ...
      ['beforeDelete', 'Author'],
      ['afterDelete', 'Author'],
      ['beforeDelete', 'Publisher'],
      ['afterDelete', 'Publisher'],
    ]);
  ...
  test('collection allows custom where and orderBy', async () => {
    const book = new Book('My Life on The Wall, part 1', new Author('name', 'email'));
    const tag1 = new BookTag('silly');
    const tag2 = new BookTag('funny');
    const tag3 = new BookTag('sick');
    const tag4 = new BookTag('strange');
    ...
    orm.em.clear();
    const ent3 = await orm.em.findOneOrFail(Book, book.uuid);
  ...
  test('many to many collection allows custom orderBy', async () => {
    ...
    const tag4 = new BookTag('strange');
    const tag5 = new BookTag('sexy');
    const tag6 = new BookTag('zupa');
    const tag7 = new BookTag('awkward');
    book1.tags.add(tag1, tag3, tag6, tag7);
    book.tags.add(tag1, tag2, tag5, tag6);
    book3.tags.add(tag2, tag4, tag5, tag7);
    author.books.add(book1, book, book3);
    await orm.em.persistAndFlush(author);
    orm.em.clear();

    const books = await orm.em.find(Book, { tags: { name: { $ne: 'funny' } } }, ['tags'], { title: QueryOrder.DESC, tags: { name: QueryOrder.ASC } });
  ...
  test('many to many with composite pk', async () => {
    ...
    book.tagsUnordered.add(tag1, tag2, tag5, tag6);
    ...
    orm.em.clear();
    const books = await orm.em.find(Book, { tagsUnordered: { name: { $ne: 'funny' } } }, ['tagsUnordered', 'perex'], { title: QueryOrder.DESC });
    expect(mock.mock.calls[0][0]).toMatch('select `e0`.*, `e0`.price * 1.19 as `price_taxed`, `e3`.`id` as `test_id` from `book` as `e0` ' +
      'left join `book_to_tag_unordered` as `e2` on `e0`.`uuid_pk` = `e2`.`book_uuid_pk` ' +
      'left join `book_tag` as `e1` on `e2`.`book_tag_id` = `e1`.`id` ' +
      'left join `test` as `e3` on `e0`.`uuid_pk` = `e3`.`book_uuid_pk` ' +
    ...
    const tags = await orm.em.find(BookTag, { booksUnordered: { title: { $ne: 'My Life on The Wall, part 3' } } }, ['booksUnordered.perex'], { name: QueryOrder.ASC });
    expect(mock.mock.calls[1][0]).toMatch('select `e0`.*, `e0`.price * 1.19 as `price_taxed`, `e1`.`book_uuid_pk`, `e1`.`book_tag_id`, `e2`.`id` as `test_id` from `book` as `e0` ' +
      'left join `book_to_tag_unordered` as `e1` on `e0`.`uuid_pk` = `e1`.`book_uuid_pk` ' +
      'left join `test` as `e2` on `e0`.`uuid_pk` = `e2`.`book_uuid_pk` ' +
      'where `e0`.`title` != ? and `e1`.`book_tag_id` in (?, ?, ?, ?, ?, ?)');
  ...
  test('filters', async () => {
    ...
    author.books.add(book1, book, book3);
    const god = new Author('God', 'hello@heaven.god');
    const bible1 = new Book('Bible', god);
    const bible2 = new Book('Bible pt. 2', god);
    const bible3 = new Book('Bible pt. 3', new Author('Lol', 'lol@lol.lol'));
    ...
    orm.em.clear();
    const books1 = await orm.em.find(Book, { title: '123' }, {
      populate: ['perex'],
    ...
    expect(mock.mock.calls[0][0]).toMatch('select `e0`.*, `e0`.price * 1.19 as `price_taxed`, `e1`.`id` as `test_id` ' +
      'from `book` as `e0` ' +
      'left join `test` as `e1` on `e0`.`uuid_pk` = `e1`.`book_uuid_pk` ' +
      'where `e0`.`title` = \'123\' and `e0`.`author_id` is not null and length(perex) > 10000 and `e0`.`price` > 1000 ' +
      'order by `e0`.`title` desc');

    const books2 = await orm.em.find(Book, { title: '123' }, {
    ...
    expect(mock.mock.calls[1][0]).toMatch('select `e0`.`uuid_pk`, `e0`.`created_at`, `e0`.`title`, `e0`.`price`, `e0`.`double`, `e0`.`meta`, `e0`.`author_id`, `e0`.`publisher_id`, `e0`.price * 1.19 as `price_taxed`, `e2`.`id` as `test_id` ' +
      'from `book` as `e0` ' +
      'left join `author` as `e1` on `e0`.`author_id` = `e1`.`id` ' +
      'left join `test` as `e2` on `e0`.`uuid_pk` = `e2`.`book_uuid_pk` ' +
      'where `e0`.`title` = \'123\' and `e1`.`name` = \'God\' and length(perex) > 10000');

    const books3 = await orm.em.find(Book, { title: '123' }, {
      filters: false,
    });
    ...
    const books4 = await orm.em.find(Book, { title: '123' }, {
      filters: true,
    });
    ...
    const b1 = await orm.em.findOne(Book, '123', {
      filters: { long: true },
    });
    ...
    const b2 = await orm.em.findOne(Book, { author: { name: 'Jon' } }, {
    ...
    await orm.em.count(Book, { author: { name: 'Jon' } }, {
      filters: { hasAuthor: false, long: true },
    });
    ...
    await orm.em.nativeUpdate(Book, '123', { title: 'b123' }, {
      filters: { hasAuthor: false, long: true },
    });
    expect(mock.mock.calls[7][0]).toMatch('update `book` set `title` = \'b123\'');

    await orm.em.nativeUpdate(Book, '123', { title: 'b123' }, {
      filters: { hasAuthor: false, long: true },
    });
    expect(mock.mock.calls[8][0]).toMatch('update `book` set `title` = \'b123\' where `uuid_pk` = \'123\' and length(perex) > 10000');

    await orm.em.nativeDelete(Book, '321', {
      filters: { hasAuthor: false, long: true },
    });
    expect(mock.mock.calls[9][0]).toMatch('delete from `book` where `uuid_pk` = \'321\' and length(perex) > 10000');

    // wut, name: 'god' should not be here?
    await expect(orm.em.find(Book, {}, {
  ...
  test('self referencing M:N (unidirectional)', async () => {
    ...
    const jon = await orm.em.findOneOrFail(Author, author.id, ['friends'], { friends: { name: QueryOrder.ASC } });
    ...
    expect(mock.mock.calls[0][0]).toMatch('select `e0`.*, `e3`.`author_id` as `address_author_id` ' +
      'from `author` as `e0` ' +
      'left join `author_to_friend` as `e2` on `e0`.`id` = `e2`.`author_1_id` ' +
      'left join `author` as `e1` on `e2`.`author_2_id` = `e1`.`id` ' +
      'left join `address2` as `e3` on `e0`.`id` = `e3`.`author_id` ' +
      'where `e0`.`id` = ? ' +
      'order by `e1`.`name` asc ' +
      'limit ?');
    expect(mock.mock.calls[1][0]).toMatch('select `e0`.*, `e1`.`author_2_id`, `e1`.`author_1_id`, `e2`.`author_id` as `address_author_id` ' +
      'from `author` as `e0` ' +
      'left join `author_to_friend` as `e1` on `e0`.`id` = `e1`.`author_2_id` ' +
      'left join `address2` as `e2` on `e0`.`id` = `e2`.`author_id` ' +
      'where `e1`.`author_1_id` in (?) ' +
      'order by `e0`.`name` asc');
    orm.em.clear();

    const jon2 = await orm.em.findOneOrFail(Author, { friends: a2.id }, ['friends'], { friends: { name: QueryOrder.ASC } });
    expect(mock.mock.calls[2][0]).toMatch('select `e0`.*, `e3`.`author_id` as `address_author_id` ' +
      'from `author` as `e0` ' +
      'left join `author_to_friend` as `e1` on `e0`.`id` = `e1`.`author_1_id` ' +
      'left join `author` as `e2` on `e1`.`author_2_id` = `e2`.`id` ' +
      'left join `address2` as `e3` on `e0`.`id` = `e3`.`author_id` ' +
      'where `e1`.`author_2_id` = ? ' +
      'order by `e2`.`name` asc ' +
      'limit ?');
    expect(mock.mock.calls[3][0]).toMatch('select `e0`.*, `e1`.`author_2_id`, `e1`.`author_1_id`, `e2`.`author_id` as `address_author_id` ' +
      'from `author` as `e0` ' +
      'left join `author_to_friend` as `e1` on `e0`.`id` = `e1`.`author_2_id` ' +
      'left join `address2` as `e2` on `e0`.`id` = `e2`.`author_id` ' +
      'where `e1`.`author_1_id` in (?) ' +
  ...
  test('self referencing M:N (owner)', async () => {
    ...
    const jon = await orm.em.findOneOrFail(Author, author.id, ['following'], { following: { name: QueryOrder.ASC } });
    expect(jon.following.isInitialized(true)).toBe(true);
    expect(jon.following.getIdentifiers()).toEqual([a1.id, a2.id, a3.id, author.id]);
    orm.em.clear();

    const jon2 = await orm.em.findOneOrFail(Author, { following: a2.id }, ['following'], { following: { name: QueryOrder.ASC } });
    expect(jon2.id).toBe(author.id);
  ...
  test('self referencing M:N (inverse)', async () => {
    ...
    const jon = await orm.em.findOneOrFail(Author, author.id, ['followers'], { followers: { name: QueryOrder.ASC } });
    expect(jon.followers.isInitialized(true)).toBe(true);
    expect(jon.followers.getIdentifiers()).toEqual([a1.id, a3.id, author.id]);

    const jon2 = await orm.em.findOneOrFail(Author, { followers: a1.id }, ['followers'], { followers: { name: QueryOrder.ASC } });
    expect(jon2.id).toBe(author.id);
  ...
  test('EM supports native insert/update/delete', async () => {
    ...
    const a = new Author('native name 1', 'native1@email.com');
    ...
    const res3 = await orm.em.nativeDelete(Author, { name: 'new native name' });
    expect(res3).toBe(1);

    const res4 = await orm.em.nativeInsert(Author, { createdAt: new Date('1989-11-17'), updatedAt: new Date('2018-10-28'), name: 'native name 2', email: 'native2@email.com' });
    expect(typeof res4).toBe('number');

    const res5 = await orm.em.nativeUpdate(Author, { name: 'native name 2' }, { name: 'new native name', updatedAt: new Date('2018-10-28') });
    expect(res5).toBe(1);

    const res6 = await orm.em.nativeUpdate<Author>('author', { name: 'new native name' }, { name: 'native name 3' });
    expect(res6).toBe(1);

    const res7 = await orm.em.nativeDelete<Author>('author', res4);
    expect(res7).toBe(1);

    const id = await orm.em.nativeInsert(Author, { name: 'native name 1', email: 'native1@email.com' });

    const res8 = await orm.em.nativeUpdate(Author, id, { friends: [id] });
    expect(res8).toBe(0);

    const res9 = await orm.em.nativeUpdate(Author, id, {});
    expect(res9).toBe(0);
  });
  ...
  test('Utils.prepareEntity changes entity to number id', async () => {
    ...
    const author = new Author('Name 2', 'e-mail2');
    author.favouriteBook = book;
    author.version = 123;
    await orm.em.persistAndFlush([author1, author, book]);
    const diff = orm.em.getComparator().diffEntities(author1, author);
  ...
  test('self referencing (1 step)', async () => {
    ...
    expect(mock.mock.calls[0][0]).toMatch('begin');
    expect(mock.mock.calls[1][0]).toMatch('insert into `author` (`created_at`, `email`, `name`, `terms_accepted`, `updated_at`) values (?, ?, ?, ?, ?)');
    expect(mock.mock.calls[2][0]).toMatch('insert into `book` (`author_id`, `created_at`, `title`, `uuid_pk`) values (?, ?, ?, ?)');
    expect(mock.mock.calls[3][0]).toMatch('insert into `book` (`author_id`, `created_at`, `title`, `uuid_pk`) values (?, ?, ?, ?)');
    expect(mock.mock.calls[4][0]).toMatch('insert into `book` (`author_id`, `created_at`, `title`, `uuid_pk`) values (?, ?, ?, ?)');
    expect(mock.mock.calls[5][0]).toMatch('update `author` set `favourite_author_id` = ?, `updated_at` = ? where `id` = ?');
    expect(mock.mock.calls[6][0]).toMatch('commit');
    expect(mock.mock.calls[7][0]).toMatch('select `e0`.*, `e1`.`author_id` as `address_author_id` from `author` as `e0` left join `address2` as `e1` on `e0`.`id` = `e1`.`author_id` where `e0`.`id` = ? limit ?');
  });
  ...
  test('self referencing 1:1 (1 step)', async () => {
    ...
    const b1 = (await orm.em.findOne(FooBar, { id: bar.id }))!;
  ...
  test('persisting entities in parallel inside forked EM with copied IM', async () => {
    ...
    const saveBook = async (title: string, author: number) => {
      ...
      const book = new Book(title, em.getReference(Author, author));
    ...
    orm.em.clear();
    const a1 = (await orm.em.findOne(Author, author.id, { populate: ['books'] }))!;
  ...
  test('EM supports smart search conditions', async () => {
    ...
    const a1 = (await orm.em.findOne(Author, { 'id:ne': 10 } as any))!;
    expect(a1).not.toBeNull();
    expect(a1.id).toBe(author.id);
    const a2 = (await orm.em.findOne(Author, { 'id>=': 1 } as any))!;
    expect(a2).not.toBeNull();
    expect(a2.id).toBe(author.id);
    const a3 = (await orm.em.findOne(Author, { 'id:nin': [2, 3, 4] } as any))!;
    expect(a3).not.toBeNull();
    expect(a3.id).toBe(author.id);
    const a4 = (await orm.em.findOne(Author, { 'id:in': [] } as any))!;
    expect(a4).toBeNull();
    const a5 = (await orm.em.findOne(Author, { 'id:nin': [] } as any))!;
    expect(a5).not.toBeNull();
    expect(a5.id).toBe(author.id);
    const a6 = (await orm.em.findOne(Author, { $and: [{ 'id:nin': [] }, { email: 'email' }] } as any))!;
  ...
  test('lookup by array', async () => {
    ...
    author.books.add(book1, book, book3);
    await orm.em.persistAndFlush(author);
    await expect(orm.em.count(Book, [book1.uuid, book.uuid, book3.uuid])).resolves.toBe(3);
    ...
    await expect(orm.em.count<Book>(Book, [book1, book, book3])).resolves.toBe(3);
    await expect(orm.em.count<any>(Book, [book1, book, book3])).resolves.toBe(3);
    const a = await orm.em.find<any>(Book, [book1, book, book3]) as Book[];
    await expect(orm.em.getRepository(Book).count([book1, book, book3])).resolves.toBe(3);
  });
  ...
  test('find by joined property', async () => {
    ...
    const logger = new Logger(mock, ['query']);
    Object.assign(orm.config, { logger });
    const res1 = await orm.em.find(Book, { author: { name: 'Jon Snow' } }, ['perex']);
    expect(res1).toHaveLength(3);
    expect(res1[0].test).toBeInstanceOf(Test);
    ...
    expect(mock.mock.calls[0][0]).toMatch('select `e0`.*, `e0`.price * 1.19 as `price_taxed`, `e2`.`id` as `test_id` ' +
      'from `book` as `e0` ' +
      'left join `author` as `e1` on `e0`.`author_id` = `e1`.`id` ' +
      'left join `test` as `e2` on `e0`.`uuid_pk` = `e2`.`book_uuid_pk` ' + // auto-joined 1:1 to get test id as book is inverse side
      'where `e1`.`name` = ?');

    orm.em.clear();
    mock.mock.calls.length = 0;
    const res2 = await orm.em.find(Book, { author: { favouriteBook: { author: { name: 'Jon Snow' } } } }, ['perex']);
    expect(res2).toHaveLength(3);
    expect(res2[0].test).toBeInstanceOf(Test);
    ...
    expect(mock.mock.calls[0][0]).toMatch('select `e0`.*, `e0`.price * 1.19 as `price_taxed`, `e4`.`id` as `test_id` ' +
      'from `book` as `e0` ' +
      'left join `author` as `e1` on `e0`.`author_id` = `e1`.`id` ' +
      'left join `book` as `e2` on `e1`.`favourite_book_uuid_pk` = `e2`.`uuid_pk` ' +
      'left join `author` as `e3` on `e2`.`author_id` = `e3`.`id` ' +
      'left join `test` as `e4` on `e0`.`uuid_pk` = `e4`.`book_uuid_pk` ' +
      'where `e3`.`name` = ?');

    orm.em.clear();
    mock.mock.calls.length = 0;
    const res3 = await orm.em.find(Book, { author: { favouriteBook: book3 } }, ['perex']);
    expect(res3).toHaveLength(3);
    expect(res3[0].test).toBeInstanceOf(Test);
    ...
    orm.em.clear();
    mock.mock.calls.length = 0;
    const res4 = await orm.em.find(Book, { author: { favouriteBook: { $or: [{ author: { name: 'Jon Snow' } }] } } }, ['perex']);
    expect(res4).toHaveLength(3);
    expect(res4[0].test).toBeInstanceOf(Test);
  ...
  test('find and count', async () => {
    ...
    const [authors1, count1] = await orm.em.findAndCount(Author, {}, { limit: 10, offset: 10, orderBy: { id: QueryOrder.ASC } });
    ...
    const [authors2, count2] = await orm.em.findAndCount(Author, {}, { limit: 10, offset: 25, fields: ['name'], orderBy: { id: QueryOrder.ASC } });
  ...
  test('query highlighting', async () => {
    ...
    if (chalk.level > 0) {
      expect(mock.mock.calls.length).toBe(3);
      expect(mock.mock.calls[0][0]).toMatch('begin');
      expect(mock.mock.calls[1][0]).toMatch('[37m[1minsert[22m[39m [37m[1minto[22m[39m [33m`author`[39m ([33m`created_at`[39m[0m,[0m [33m`email`[39m[0m,[0m [33m`name`[39m[0m,[0m [33m`terms_accepted`[39m[0m,[0m [33m`updated_at`[39m) [37m[1mvalues[22m[39m (?[0m,[0m ?[0m,[0m ?[0m,[0m ?[0m,[0m ?)');
      expect(mock.mock.calls[2][0]).toMatch('commit');
    }
  ...
  test('read replicas', async () => {
    ...
    author.born = new Date('1990-03-23');
    author.books.add(new Book('B', author));
    await orm.em.persistAndFlush(author);
    expect(mock.mock.calls[0][0]).toMatch(/begin.*via write connection '127\.0\.0\.1'/);
    expect(mock.mock.calls[1][0]).toMatch(/insert into `author`.*via write connection '127\.0\.0\.1'/);
    expect(mock.mock.calls[2][0]).toMatch(/insert into `book`.*via write connection '127\.0\.0\.1'/);
    expect(mock.mock.calls[3][0]).toMatch(/commit.*via write connection '127\.0\.0\.1'/);

    orm.em.clear();
    author = (await orm.em.findOne(Author, author))!;
    await orm.em.findOne(Author, author, { refresh: true });
    await orm.em.findOne(Author, author, { refresh: true });
    expect(mock.mock.calls[4][0]).toMatch(/select `e0`\.\*, `e1`\.`author_id` as `address_author_id` from `author` as `e0` left join `address2` as `e1` on `e0`\.`id` = `e1`\.`author_id` where `e0`.`id` = \? limit \?.*via read connection 'read-\d'/);
    expect(mock.mock.calls[5][0]).toMatch(/select `e0`\.\*, `e1`\.`author_id` as `address_author_id` from `author` as `e0` left join `address2` as `e1` on `e0`\.`id` = `e1`\.`author_id` where `e0`.`id` = \? limit \?.*via read connection 'read-\d'/);
    expect(mock.mock.calls[6][0]).toMatch(/select `e0`\.\*, `e1`\.`author_id` as `address_author_id` from `author` as `e0` left join `address2` as `e1` on `e0`\.`id` = `e1`\.`author_id` where `e0`.`id` = \? limit \?.*via read connection 'read-\d'/);

    author.name = 'Jon Blow';
    await orm.em.flush();
    expect(mock.mock.calls[7][0]).toMatch(/begin.*via write connection '127\.0\.0\.1'/);
    expect(mock.mock.calls[8][0]).toMatch(/update `author` set `name` = \?, `updated_at` = \? where `id` = \?.*via write connection '127\.0\.0\.1'/);
    expect(mock.mock.calls[9][0]).toMatch(/commit.*via write connection '127\.0\.0\.1'/);

    const qb = orm.em.createQueryBuilder(Author, 'a', 'write');
    await qb.select('*').where({ name: /.*Blow/ }).execute();
    expect(mock.mock.calls[10][0]).toMatch(/select `a`.* from `author` as `a` where `a`.`name` like \?.*via write connection '127\.0\.0\.1'/);

    await orm.em.transactional(async em => {
      const book = await em.findOne(Book, { title: 'B' });
    ...
    expect(mock.mock.calls[12][0]).toMatch(/select `e0`.*, `e1`\.`id` as `test_id` from `book` as `e0` left join `test` as `e1` on `e0`.`uuid_pk` = `e1`.`book_uuid_pk` where `e0`.`title` = \?.*via write connection '127\.0\.0\.1'/);
    expect(mock.mock.calls[13][0]).toMatch(/update `author` set `name` = \?, `favourite_book_uuid_pk` = \?, `updated_at` = \? where `id` = \?.*via write connection '127\.0\.0\.1'/);
  ...
  test('datetime is stored in correct timezone', async () => {
    ...
    const res = await orm.em.getConnection().execute<{ born: string }[]>(`select date_format(born, '%Y-%m-%d %T.%f') as born from author where id = ${author.id}`);
    expect(res[0].born).toBe('2000-01-01 00:00:00.000000');
    const a = await orm.em.findOneOrFail(Author, author.id);
  ...
  test('find where property is not null ($ne operator)', async () => {
    const author = new Author('n', 'e');
    const book1 = new Book('b1', author);
    book1.publisher = wrap(new Publisher('p')).toReference();
    const book = new Book('b2', author);
    await orm.em.persistAndFlush([book1, book]);
    orm.em.clear();

    const res = await orm.em.find(Book, { publisher: { $ne: null } });
    expect(res.length).toBe(1);
    expect(res[0].publisher).toBeInstanceOf(Reference);
    expect(res[0].publisher!.unwrap()).toBeInstanceOf(Publisher);
  ...
  test('find with different schema', async () => {
    ...
    const res1 = await orm.em.find(Book, { publisher: { $ne: null } }, { schema: 'mikro_orm_test_schema_2', populate: ['perex'] });
    const res2 = await orm.em.find(Book, { publisher: { $ne: null } }, ['perex']);
    expect(mock.mock.calls[0][0]).toMatch('select `e0`.*, `e0`.price * 1.19 as `price_taxed`, `e1`.`id` as `test_id` from `mikro_orm_test_schema_2`.`book` as `e0` left join `mikro_orm_test_schema_2`.`test` as `e1` on `e0`.`uuid_pk` = `e1`.`book_uuid_pk` where `e0`.`publisher_id` is not null');
    expect(mock.mock.calls[1][0]).toMatch('select `e0`.*, `e0`.price * 1.19 as `price_taxed`, `e1`.`id` as `test_id` from `book` as `e0` left join `test` as `e1` on `e0`.`uuid_pk` = `e1`.`book_uuid_pk` where `e0`.`publisher_id` is not null');
  ...
  test('calling Collection.init in parallel', async () => {
    const author = new Author('n', 'e');
    author.books.add(new Book('b1', author));
    author.books.add(new Book('b2', author));
    await orm.em.persistAndFlush(author);
    orm.em.clear();

    const author2 = await orm.em.findOneOrFail(Author, author.id);
  ...
  test('adding items to not initialized collection', async () => {
    ...
    const b2 = new Book('Bible 2', a);
    const b3 = new Book('Bible 3', a);
    a.books.add(b2, b3);
    await orm.em.flush();
    orm.em.clear();
    ...
    const book = await orm.em.findOneOrFail(Book, b3.uuid);
    tag.books.add(book);
    await orm.em.flush();
    orm.em.clear();
  });
  ...
  test('pagination', async () => {
    for (let i = 1; i <= 10; i++) {
      ...
      const god = new Author(`God ${num}`, `hello${num}@heaven.god`);
      new Book(`Bible ${num}.1`, god);
      new Book(`Bible ${num}.2`, god);
      new Book(`Bible ${num}.3`, god);
      orm.em.persist(god);
    }
    ...
    const res1 = await orm.em.find(Author, { books: { title: /^Bible/ } }, {
    ...
    const res2 = await orm.em.find(Author, { books: { title: /^Bible/ } }, {
      orderBy: { name: QueryOrder.ASC, books: { title: QueryOrder.ASC } },
      offset: 3,
      limit: 5,
    ...
    const res3 = await orm.em.find(Author, { books: { title: /^Bible/ } }, {
      orderBy: { name: QueryOrder.ASC, books: { title: QueryOrder.ASC } },
      limit: 5,
      flags: [QueryFlag.PAGINATE],
    });
  ...
  test('formulas', async () => {
    ...
    const b = await orm.em.findOneOrFail(Book, { author: { name: 'God' } });
  ...
  test('exceptions', async () => {
    await orm.em.nativeInsert(Author, { name: 'author', email: 'email' });
    await expect(orm.em.nativeInsert(Author, { name: 'author', email: 'email' })).rejects.toThrow(UniqueConstraintViolationException);
    await expect(orm.em.nativeInsert('not_existing', { foo: 'bar' })).rejects.toThrow(TableNotFoundException);
    await expect(orm.em.execute('create table author (foo text not null)')).rejects.toThrow(TableExistsException);
    await expect(orm.em.execute('foo bar 123')).rejects.toThrow(SyntaxErrorException);
    await expect(orm.em.execute('select id from author, foo_bar2')).rejects.toThrow(NonUniqueFieldNameException);
    await expect(orm.em.execute('select uuid from author')).rejects.toThrow(InvalidFieldNameException);
  ...
  test('GH 625', async () => {
    ...
    const res = await orm.em.findOne(Author, { id: a.id, termsAccepted: true });
    expect(res).toBeNull();
  });

  test('em.execute()', async () => {
    const res1 = await orm.em.execute('insert into author (name, email) values (?, ?)', ['name', 'email']);
  ...
  test('lazy scalar properties', async () => {
    ...
    expect(mock.mock.calls[0][0]).toMatch('select `e0`.*, `e1`.`author_id` as `address_author_id` from `author` as `e0` left join `address2` as `e1` on `e0`.`id` = `e1`.`author_id`');
