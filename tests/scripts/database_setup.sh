...
if [[ -z "$POSTGRES_CONTAINER_NAME" ]]; then
    echo "POSTGRESQL container not running. Please check the name of the container."
    exit
fi
docker exec $POSTGRES_CONTAINER_NAME bash -c "psql -c 'CREATE DATABASE mikro_orm_test;' -U postgres"
