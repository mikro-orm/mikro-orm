...
describe('EntityManagerOracle', () => {
  ...
  beforeAll(async () => {
    orm = await MikroORM.init({
      ...
      entities: ['./entities-sql', '!./entities-sql/Label2.ts', '!./entities-sql/index.ts'],
  ...
  // FIXME
  test.skip('raw query with array param', async () => {
  ...
  test('should return oracledb driver', async () => {
    ...
    expect(mock.mock.calls[1][0]).toMatch(`begin execute immediate 'insert into "publisher2_tests" ("test2_id", "publisher2_id") values (?, ?) returning "id" into :out_id' using out :out_id__0; execute immediate 'insert into "publisher2_tests" ("test2_id", "publisher2_id") values (?, ?) returning "id" into :out_id' using out :out_id__1; execute immediate 'insert into "publisher2_tests" ("test2_id", "publisher2_id") values (?, ?) returning "id" into :out_id' using out :out_id__2; execute immediate 'insert into "publisher2_tests" ("test2_id", "publisher2_id") values (?, ?) returning "id" into :out_id' using out :out_id__3; execute immediate 'insert into "publisher2_tests" ("test2_id", "publisher2_id") values (?, ?) returning "id" into :out_id' using out :out_id__4; execute immediate 'insert into "publisher2_tests" ("test2_id", "publisher2_id") values (?, ?) returning "id" into :out_id' using out :out_id__5; execute immediate 'insert into "publisher2_tests" ("test2_id", "publisher2_id") values (?, ?) returning "id" into :out_id' using out :out_id__6; execute immediate 'insert into "publisher2_tests" ("test2_id", "publisher2_id") values (?, ?) returning "id" into :out_id' using out :out_id__7; end;`);
  ...
  test('collection loads items after savepoint should not fail', async () => {
    ...
    expect(mock.mock.calls[3][0]).toMatch(`select "p0"."id", "p0"."test2_id", "p0"."publisher2_id", "t1"."id" "t1__id", "t1"."name" "t1__name", "t1"."book_uuid_pk" "t1__book_uuid_pk", "t1"."parent_id" "t1__parent_id", "t1"."version" "t1__version", "b2"."uuid_pk" "b2__uuid_pk" from "publisher2_tests" "p0" inner join "test2" "t1" on "p0"."test2_id" = "t1"."id" left join "book2" "b2" on "t1"."book_uuid_pk" = "b2"."uuid_pk" where "p0"."publisher2_id" in (?) order by "p0"."id" asc for update of "p0"."id", "t1"."id"`);
  ...
  test('locking and select-in population (GH #1670)', async () => {
    ...
    expect(mock.mock.calls[2][0]).toMatch(`select "a0".*, "f1"."uuid_pk" "f1__uuid_pk" from "author2" "a0" left join "book2" "f1" on "a0"."favourite_book_uuid_pk" = "f1"."uuid_pk" and "f1"."author_id" is not null where "a0"."id" in (?) and "a0"."id" is not null for update of "a0"."id" skip locked`);
    expect(mock.mock.calls[3][0]).toMatch(`select "b0"."order", "b0"."book_tag2_id", "b0"."book2_uuid_pk", "b1"."id" "b1__id", "b1"."name" "b1__name" from "book2_tags" "b0" inner join "book_tag2" "b1" on "b0"."book_tag2_id" = "b1"."id" where "b0"."book2_uuid_pk" in (?, ?, ?) order by "b0"."order" asc for update skip locked`);
  ...
  test('populating many to many relation', async () => {
    ...
    expect(mock.mock.calls[1][0]).toMatch('select "p0"."id", "p0"."test2_id", "p0"."publisher2_id" from "publisher2_tests" "p0" where "p0"."publisher2_id" in (1, 2) order by "p0"."id" asc');
    mock.mockReset();
    orm.em.clear();
    ...
    expect(mock.mock.calls).toHaveLength(1);
    expect(mock.mock.calls[0][0]).toMatch('select "p0"."id", "p0"."test2_id", "p0"."publisher2_id" from "publisher2_tests" "p0" where "p0"."publisher2_id" in (1) order by "p0"."id" asc');
  ...
  test('one to many relation (ref: true)', async () => {
    ...
    expect(mock).toHaveBeenCalledTimes(2);
    expect(mock.mock.calls[0][0]).toMatch('select "a0".*, "f1"."uuid_pk" "f1__uuid_pk" from "author2" "a0" left join "book2" "f1" on "a0"."favourite_book_uuid_pk" = "f1"."uuid_pk" and "f1"."author_id" is not null where "a0"."id" = 1 fetch next 1 rows');
  ...
  test('populating dirty collections will merge the items and keep it dirty', async () => {
    ...
    expect(mock.mock.calls[2][0]).toMatch(`commit`);
    expect(mock.mock.calls[3][0]).toMatch(`select "b0"."uuid_pk", "b0"."created_at", "b0"."isbn", "b0"."title", "b0"."price", "b0"."double", "b0"."meta", "b0"."author_id", "b0"."publisher_id", "b0"."price" * 1.19 as "price_taxed" from "book2" "b0" where "b0"."author_id" is not null and "b0"."author_id" in (?) order by "b0"."title" asc`);
  });
  ...
  test('find with custom function', async () => {
    ...
    expect(mock.mock.calls[0][0]).toMatch(`select "b0".*, "b0"."price" * 1.19 as "price_taxed" from "book2" "b0" inner join "author2" "a1" on "b0"."author_id" = "a1"."id" where "b0"."author_id" is not null and "a1"."age" like '%2%' and upper("title") in ('B1', 'B2')`);
    orm.em.clear();
  ...
  test('exceptions', async () => {
    ...
    await expect(orm.driver.nativeInsert(Author2, { name: 'author', email: 'email' })).rejects.toThrow(UniqueConstraintViolationException);
    await expect(orm.driver.nativeInsert(Author2, {})).rejects.toThrow(NotNullConstraintViolationException);
    await expect(orm.driver.execute('create table author2 (foo clob not null)')).rejects.toThrow(TableExistsException);
    await expect(orm.driver.execute('foo bar 123')).rejects.toThrow(SyntaxErrorException);
    await expect(orm.driver.execute('select "id" from "author2", "foo_bar2"')).rejects.toThrow(NonUniqueFieldNameException);
  ...
  test('required fields validation', async () => {
    ...
    await expect(orm.em.persist(jon).flush()).rejects.toThrow(`ORA-01400: cannot insert NULL into ("mikro_orm_test"."author2"."email")`);
