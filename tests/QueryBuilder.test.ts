...
describe('QueryBuilder', () => {
  ...
  test('select query', async () => {
    ...
    qb1
      ...
      .orderBy({
        [`(point(location_latitude, location_longitude) <@> point(${53}, ${9}))`]: 'ASC',
      });
    ...
    qb2
      ...
      .where({ name: 'test 123', type: PublisherType.GLOBAL })
      .orderBy({
        [`(point(location_latitude, location_longitude) <@> point(${53.46}, ${9.9}))`]: 'ASC',
      });
  ...
  test('select multiple andWhere', async () => {
    const qb = orm.em.createQueryBuilder(Publisher2);
    qb.select('*').where({ name: 'test 123' }).andWhere({ type: PublisherType.GLOBAL }).andWhere({ name: 'test 321' }).andWhere({ name: 'lol 321' }).limit(2, 1);
  ...
  test('select leftJoin 1:1 owner', async () => {
    ...
    qb.select(['fb.*', 'fz.*']).leftJoin('fb.baz', 'fz').where({ 'fz.name': 'test 123' }).limit(2, 1);
    const sql = 'select `fb`.*, `fz`.*, (select 123) as `random` from `foo_bar2` as `fb` ' + 'left join `foo_baz2` as `fz` on `fb`.`baz_id` = `fz`.`id` ' + 'where `fz`.`name` = ? ' + 'limit ? offset ?';
  ...
  test('select leftJoin 1:1 inverse', async () => {
    const qb = orm.em.createQueryBuilder(FooBaz2, 'fz');
    qb.select(['fb.*', 'fz.*']).leftJoin('fz.bar', 'fb').where({ 'fb.name': 'test 123' }).limit(2, 1);
    const sql = 'select `fb`.*, `fz`.* from `foo_baz2` as `fz` ' + 'left join `foo_bar2` as `fb` on `fz`.`id` = `fb`.`baz_id` ' + 'where `fb`.`name` = ? ' + 'limit ? offset ?';
    expect(qb.getQuery()).toEqual(sql);
    expect(qb.getParams()).toEqual(['test 123', 2, 1]);
  });
  ...
  test('select leftJoin m:1', async () => {
    ...
    qb.select(['a.*', 'b.*']).leftJoin('b.author', 'a').where({ 'a.name': 'test 123' });
    const sql = 'select `a`.*, `b`.*, `b`.price * 1.19 as `price_taxed` from `book2` as `b` ' + 'left join `author2` as `a` on `b`.`author_id` = `a`.`id` ' + 'where `a`.`name` = ?';
    expect(qb.getQuery()).toEqual(sql);
  ...
  test('select leftJoin 1:m', async () => {
    ...
    qb.select(['a.*', 'b.*']).leftJoin('a.books', 'b').where({ 'b.title': 'test 123' });
  ...
  test('select leftJoin 1:m with $not in extra condition (GH #3504)', async () => {
    ...
    const sql = 'select `a`.*, `b`.* from `author2` as `a` ' + 'left join `book2` as `b` on `a`.`id` = `b`.`author_id` and not `b`.`title` = ? ' + 'where `b`.`title` = ?';
    expect(qb.getQuery()).toEqual(sql);
  ...
  test('select leftJoin 1:m with custom sql fragments', async () => {
    ...
    qb.select(['a.*', 'b.*'])
      ...
      .where({ 'b.title': 'test 123' });
    const sql = 'select `a`.*, `b`.* from `author2` as `a` ' + 'left join `book2` as `b` on `a`.`id` = `b`.`author_id` and json_contains(`a`.`meta`, ?) and json_contains(`a`.`meta`, ?) = ? and lower(b.bar) = ? ' + 'where `b`.`title` = ?';
    expect(qb.getQuery()).toEqual(sql);
  ...
  test('select leftJoin m:n owner', async () => {
    const qb = orm.em.createQueryBuilder(Book2, 'b');
    qb.select(['b.*', 't.*']).leftJoin('b.tags', 't').where({ 't.name': 'test 123' });
    const sql = 'select `b`.*, `t`.*, `b`.price * 1.19 as `price_taxed` from `book2` as `b` ' + 'left join `book2_tags` as `e1` on `b`.`uuid_pk` = `e1`.`book2_uuid_pk` ' + 'left join `book_tag2` as `t` on `e1`.`book_tag2_id` = `t`.`id` ' + 'where `t`.`name` = ?';
    expect(qb.getQuery()).toEqual(sql);
    expect(qb.getParams()).toEqual(['test 123']);
  });
  ...
  test('select leftJoin m:n inverse', async () => {
    ...
    qb.select(['b.*', 't.*']).leftJoin('t.books', 'b').where({ 'b.title': 'test 123' });
    const sql = 'select `b`.*, `t`.* from `book_tag2` as `t` ' + 'left join `book2_tags` as `e1` on `t`.`id` = `e1`.`book_tag2_id` ' + 'left join `book2` as `b` on `e1`.`book2_uuid_pk` = `b`.`uuid_pk` ' + 'where `b`.`title` = ?';
    expect(qb.getQuery()).toEqual(sql);
    expect(qb.getParams()).toEqual(['test 123']);
  });
  ...
  test('select join and leftJoin combined', async () => {
    ...
    qb.select(['p.*', 'b.*', 'a.*', 't.*']).leftJoin('books', 'b').join('b.author', 'a').join('b.tags', 't').where({ 'p.name': 'test 123', 'b.title': /3$/ });
    const sql =
  ...
  test('select with leftJoin for same property multiple times', async () => {
    const qb = orm.em.createQueryBuilder(Publisher2, 'p');
    qb.select(['p.*', 'b.*', 'b2.*']).leftJoin('books', 'b').leftJoin('books', 'b2').where({ 'b.title': 'test 123', 'b2.title': /3$/ });
    const sql = 'select `p`.*, `b`.*, `b2`.* ' + 'from `publisher2` as `p` ' + 'left join `book2` as `b` on `p`.`id` = `b`.`publisher_id` ' + 'left join `book2` as `b2` on `p`.`id` = `b2`.`publisher_id` ' + 'where `b`.`title` = ? and `b2`.`title` like ?';
    expect(qb.getQuery()).toEqual(sql);
    expect(qb.getParams()).toEqual(['test 123', '%3']);
  });
  ...
  test('select with custom expression', async () => {
    ...
    const qb2 = orm.em.createQueryBuilder(Book2);
    qb2.select('*').where({
      [expr((a) => `json_contains(\`${a}\`.\`meta\`, ?) = ?`)]: [{ foo: 'baz' }, false],
    });
  ...
  test('GH #1668', async () => {
    ...
    qb1.select(['floor(`a`.`age`) as `books_total`']).groupBy('booksTotal').orderBy({ booksTotal: QueryOrder.ASC });
    ...
    qb2.select(['floor(`a`.`age`) as `code`']).groupBy('code').orderBy({ code: QueryOrder.ASC });
  ...
  test('select by m:n', async () => {
    ...
    expect(qb.getQuery()).toEqual('select `e0`.*, `e0`.price * 1.19 as `price_taxed` from `book2` as `e0` ' + 'left join `book2_tags` as `e1` on `e0`.`uuid_pk` = `e1`.`book2_uuid_pk` ' + 'where `e1`.`book_tag2_id` = ?');
    expect(qb.getParams()).toEqual(['123']);
  });
  ...
  test('select by m:n inversed', async () => {
    ...
    expect(qb.getQuery()).toEqual('select `e0`.* from `book_tag2` as `e0` ' + 'left join `book2_tags` as `e1` on `e0`.`id` = `e1`.`book_tag2_id` ' + 'where `e1`.`book2_uuid_pk` = ?');
    expect(qb.getParams()).toEqual(['123']);
  });
  ...
  test('select by m:n inverse side (that is not defined as property) via populate', async () => {
    ...
    qb.select('*')
      .populate([{ field: 'publisher2_tests' }])
      .where({ 'publisher2_tests.Publisher2_owner': { $in: [1, 2] } })
      .orderBy({ 'publisher2_tests.id': QueryOrder.ASC });
  ...
  test('select by m:n self reference owner', async () => {
    const qb = orm.em.createQueryBuilder(Author2);
    qb.select('*')
      .populate([{ field: 'author2_following' }])
      .where({ 'author2_following.Author2_owner': { $in: [1, 2] } });
  ...
  test('select by m:n self reference inverse', async () => {
    ...
    qb.select('*')
      .populate([{ field: 'author2_following' }])
      .where({ 'author2_following.Author2_inverse': { $in: [1, 2] } });
  ...
  test('select by m:n with composite keys', async () => {
    ...
    qb.select('*')
      .populate([{ field: 'user2_cars' }])
      .where({
        'user2_cars.Car2_inverse': {
          $in: [
            [1, 2],
            [3, 4],
          ],
    ...
    const sql =
      'select `e0`.*, `e1`.`user2_first_name` as `fk__user2_first_name`, `e1`.`user2_last_name` as `fk__user2_last_name`, `e1`.`car2_name` as `fk__car2_name`, `e1`.`car2_year` as `fk__car2_year` ' +
  ...
  test('select distinct id with left join', async () => {
    const qb = orm.em.createQueryBuilder(BookTag2, 't');
    qb.select(['distinct `b`.`uuid_pk`', 'b.*', 't.*']).leftJoin('t.books', 'b').where({ 'b.title': 'test 123' });
    const sql = 'select distinct `b`.`uuid_pk`, `b`.*, `t`.* from `book_tag2` as `t` ' + 'left join `book2_tags` as `e1` on `t`.`id` = `e1`.`book_tag2_id` ' + 'left join `book2` as `b` on `e1`.`book2_uuid_pk` = `b`.`uuid_pk` ' + 'where `b`.`title` = ?';
    expect(qb.getQuery()).toEqual(sql);
    expect(qb.getParams()).toEqual(['test 123']);
  });
  ...
  test('select distinct via flag', async () => {
    const qb = orm.em.createQueryBuilder(BookTag2, 't');
    qb.select(['b.uuid', 'b.*', 't.*'], true).leftJoin('t.books', 'b').where({ 'b.title': 'test 123' });
    const sql = 'select distinct `b`.`uuid_pk`, `b`.*, `t`.* from `book_tag2` as `t` ' + 'left join `book2_tags` as `e1` on `t`.`id` = `e1`.`book_tag2_id` ' + 'left join `book2` as `b` on `e1`.`book2_uuid_pk` = `b`.`uuid_pk` ' + 'where `b`.`title` = ?';
    expect(qb.getQuery()).toEqual(sql);
    expect(qb.getParams()).toEqual(['test 123']);
  });
  ...
  test('select where string literal', async () => {
    const qb = orm.em.createQueryBuilder(BookTag2, 't');
    qb.select(['b.*', 't.*']).leftJoin('t.books', 'b').where('b.title = ? or b.title = ?', ['test 123', 'lol 321']).andWhere('1 = 1').orWhere('1 = 2');
    const sql = 'select `b`.*, `t`.* from `book_tag2` as `t` ' + 'left join `book2_tags` as `e1` on `t`.`id` = `e1`.`book_tag2_id` ' + 'left join `book2` as `b` on `e1`.`book2_uuid_pk` = `b`.`uuid_pk` ' + 'where (((b.title = ? or b.title = ?) and (1 = 1)) or (1 = 2))';
    expect(qb.getQuery()).toEqual(sql);
  ...
  test('select with group by and having', async () => {
    const qb = orm.em.createQueryBuilder(BookTag2, 't');
    qb.select(['b.*', 't.*', 'count(t.id) as tags']).leftJoin('t.books', 'b').where('b.title = ? or b.title = ?', ['test 123', 'lol 321']).groupBy(['b.uuid', 't.id']).having('tags > ?', [0]);
    const sql =
  ...
  test('select with group by and having with object', async () => {
    ...
    qb.select(['b.*', 't.*', 'count(t.id) as tags'])
      ...
      .groupBy(['b.uuid', 't.id'])
      .having({
        $or: [{ 'b.uuid': '...', 'count(t.id)': { $gt: 0 } }, { 'b.title': 'my title' }],
      });
    const sql =
      'select `b`.*, `t`.*, count(t.id) as tags from `book_tag2` as `t` ' +
      'left join `book2_tags` as `e1` on `t`.`id` = `e1`.`book_tag2_id` ' +
  ...
  test('select with operator (and)', async () => {
    const qb = orm.em.createQueryBuilder(Test2);
    qb.select('*').where({
      $and: [{ id: { $in: [1, 2, 7] } }, { id: { $nin: [3, 4] } }, { id: { $gt: 5 } }, { id: { $lt: 10 } }, { id: { $gte: 7 } }, { id: { $lte: 8 } }, { id: { $ne: 9 } }, { $not: { id: { $eq: 10 } } }],
    });
    expect(qb.getQuery()).toEqual('select `e0`.* from `test2` as `e0` ' + 'where `e0`.`id` in (?, ?, ?) ' + 'and `e0`.`id` not in (?, ?) ' + 'and `e0`.`id` > ? ' + 'and `e0`.`id` < ? ' + 'and `e0`.`id` >= ? ' + 'and `e0`.`id` <= ? ' + 'and `e0`.`id` != ? ' + 'and not (`e0`.`id` = ?)');
  ...
  test('select with operator (or)', async () => {
    const qb = orm.em.createQueryBuilder(Test2);
    qb.select('*').where({
      $or: [{ id: { $in: [1, 2, 7] } }, { id: { $nin: [3, 4] } }, { id: { $gt: 5 } }, { id: { $lt: 10 } }, { id: { $gte: 7 } }, { id: { $lte: 8 } }, { id: { $ne: 9 } }, { $not: { id: { $eq: 10 } } }],
    });
    expect(qb.getQuery()).toEqual('select `e0`.* from `test2` as `e0` ' + 'where (`e0`.`id` in (?, ?, ?) ' + 'or `e0`.`id` not in (?, ?) ' + 'or `e0`.`id` > ? ' + 'or `e0`.`id` < ? ' + 'or `e0`.`id` >= ? ' + 'or `e0`.`id` <= ? ' + 'or `e0`.`id` != ? ' + 'or not (`e0`.`id` = ?))');
    expect(qb.getParams()).toEqual([1, 2, 7, 3, 4, 5, 10, 7, 8, 9, 10]);
  });
  ...
  test('select with smart query conditions', async () => {
    ...
    expect(qb.getQuery()).toEqual('select `e0`.* from `test2` as `e0` ' + 'where `e0`.`version` > ? ' + 'and `e0`.`version` < ? ' + 'and `e0`.`version` >= ? ' + 'and `e0`.`version` <= ? ' + 'and `e0`.`version` != ? ' + 'and `e0`.`version` in (?, ?) ' + 'and `e0`.`version` not in (?, ?)');
  ...
  test('select with deep where condition', async () => {
    ...
    expect(qb5.getQuery()).toEqual('select `e0`.*, `e0`.price * 1.19 as `price_taxed` ' + 'from `book2` as `e0` ' + 'left join `book2_tags` as `e1` on `e0`.`uuid_pk` = `e1`.`book2_uuid_pk` ' + 'where `e1`.`book_tag2_id` in (?, ?, ?)');
    expect(qb5.getParams()).toEqual(['1', '2', '3']);

    // m:n owner
    const qb6 = orm.em.createQueryBuilder(Book2);
    qb6.select('*').where({ tags: { name: 'Tag 3' } });
    expect(qb6.getQuery()).toEqual('select `e0`.*, `e0`.price * 1.19 as `price_taxed` ' + 'from `book2` as `e0` ' + 'left join `book2_tags` as `e2` on `e0`.`uuid_pk` = `e2`.`book2_uuid_pk` ' + 'left join `book_tag2` as `e1` on `e2`.`book_tag2_id` = `e1`.`id` ' + 'where `e1`.`name` = ?');
    ...
    expect(qb7.getQuery()).toEqual('select `e0`.* ' + 'from `book_tag2` as `e0` ' + 'left join `book2_tags` as `e1` on `e0`.`id` = `e1`.`book_tag2_id` ' + 'where `e1`.`book2_uuid_pk` in (?, ?, ?)');
    expect(qb7.getParams()).toEqual(['1', '2', '3']);

    // m:n inverse
    const qb8 = orm.em.createQueryBuilder(BookTag2);
    qb8.select('*').where({ books: { title: 'Book 123' } });
    expect(qb8.getQuery()).toEqual('select `e0`.* ' + 'from `book_tag2` as `e0` ' + 'left join `book2_tags` as `e2` on `e0`.`id` = `e2`.`book_tag2_id` ' + 'left join `book2` as `e1` on `e2`.`book2_uuid_pk` = `e1`.`uuid_pk` ' + 'where `e1`.`title` = ?');
    ...
    expect(qb9.getQuery()).toEqual('select `e0`.*, (select 123) as `random` from `foo_bar2` as `e0` ' + 'left join `foo_bar2` as `e1` on `e0`.`foo_bar_id` = `e1`.`id` ' + 'left join `foo_baz2` as `e2` on `e1`.`baz_id` = `e2`.`id` ' + 'where `e2`.`name` = ?');
    ...
    expect(qb11.getQuery()).toEqual('select `e0`.* from `foo_baz2` as `e0` ' + 'left join `foo_bar2` as `e1` on `e0`.`id` = `e1`.`baz_id` ' + 'where `e1`.`name` = ?');
    ...
    expect(qb12.getQuery()).toEqual('select `e0`.*, (select 123) as `random`, (select 456) as `lazy_random` from `foo_bar2` as `e0` ' + 'left join `foo_bar2` as `e1` on `e0`.`foo_bar_id` = `e1`.`id` ' + 'left join `foo_baz2` as `e2` on `e1`.`baz_id` = `e2`.`id` ' + 'where `e2`.`name` = ?');
  ...
  test('select with deep where condition with self-reference', async () => {
    ...
    expect(qb1.getQuery()).toEqual(
      'select `e0`.*, `e0`.price * 1.19 as `price_taxed` from `book2` as `e0` ' + 'left join `author2` as `e1` on `e0`.`author_id` = `e1`.`id` ' + 'left join `author2` as `e2` on `e1`.`favourite_author_id` = `e2`.`id` ' + 'where `e2`.`name` = ? and `e2`.`terms_accepted` = ?'
    );
    expect(qb1.getParams()).toEqual(['Jon Snow', true]);
  });
  ...
  test('select with deep order by', async () => {
    ...
    expect(qb5.getQuery()).toEqual('select `e0`.*, `e0`.price * 1.19 as `price_taxed` ' + 'from `book2` as `e0` ' + 'left join `book2_tags` as `e2` on `e0`.`uuid_pk` = `e2`.`book2_uuid_pk` ' + 'left join `book_tag2` as `e1` on `e2`.`book_tag2_id` = `e1`.`id` ' + 'order by `e1`.`name` desc');
  });
  ...
  test('select with populate and join of m:n', async () => {
    ...
    qb.select('*')
      .populate([{ field: 'tags' }])
      .leftJoin('tags', 't');
    expect(qb.getQuery()).toEqual('select `e0`.*, `e0`.price * 1.19 as `price_taxed` ' + 'from `book2` as `e0` ' + 'left join `book2_tags` as `e1` on `e0`.`uuid_pk` = `e1`.`book2_uuid_pk` ' + 'left join `book_tag2` as `t` on `e1`.`book_tag2_id` = `t`.`id`');
  });
  ...
  test('select with deep where and deep order by', async () => {
    ...
    expect(qb5.getQuery()).toEqual(
      'select `e0`.*, `e0`.price * 1.19 as `price_taxed` ' + 'from `book2` as `e0` ' + 'left join `book2_tags` as `e2` on `e0`.`uuid_pk` = `e2`.`book2_uuid_pk` ' + 'left join `book_tag2` as `e1` on `e2`.`book_tag2_id` = `e1`.`id` ' + 'where `e1`.`name` = ? order by `e1`.`name` desc'
    );
  ...
  test('select with deep where condition with operators', async () => {
    ...
    expect(qb0.getQuery()).toEqual('select `e0`.*, `e0`.price * 1.19 as `price_taxed` from `book2` as `e0` ' + 'left join `author2` as `e1` on `e0`.`author_id` = `e1`.`id` ' + 'where (`e1`.`name` = ? or `e1`.`email` like ?)');
    expect(qb0.getParams()).toEqual(['Jon Snow 1', 'snow@%']);

    const qb1 = orm.em.createQueryBuilder(Book2);
    qb1.select('*').where({
      $or: [{ author: { name: 'Jon Snow 1', termsAccepted: true } }, { author: { name: 'Jon Snow 2' } }],
    });
    expect(qb1.getQuery()).toEqual('select `e0`.*, `e0`.price * 1.19 as `price_taxed` ' + 'from `book2` as `e0` ' + 'left join `author2` as `e1` on `e0`.`author_id` = `e1`.`id` ' + 'where ((`e1`.`name` = ? and `e1`.`terms_accepted` = ?) or `e1`.`name` = ?)');
    expect(qb1.getParams()).toEqual(['Jon Snow 1', true, 'Jon Snow 2']);

    const qb2 = orm.em.createQueryBuilder(Book2);
    qb2.select('*').where({
      $or: [{ author: { $or: [{ name: 'Jon Snow 1' }, { email: /^snow@/ }] } }, { publisher: { name: 'My Publisher' } }],
    });
    expect(qb2.getQuery()).toEqual(
      'select `e0`.*, `e0`.price * 1.19 as `price_taxed` from `book2` as `e0` ' + 'left join `author2` as `e1` on `e0`.`author_id` = `e1`.`id` ' + 'left join `publisher2` as `e2` on `e0`.`publisher_id` = `e2`.`id` ' + 'where (((`e1`.`name` = ? or `e1`.`email` like ?)) or `e2`.`name` = ?)'
    );
    ...
    qb3.select('*').where({
      $or: [
        {
          author: {
            $or: [{ name: { $in: ['Jon Snow 1', 'Jon Snow 2'] } }, { email: /^snow@/ }],
          },
        },
    ...
    expect(qb3.getQuery()).toEqual(
      'select `e0`.*, `e0`.price * 1.19 as `price_taxed` from `book2` as `e0` ' + 'left join `author2` as `e1` on `e0`.`author_id` = `e1`.`id` ' + 'left join `publisher2` as `e2` on `e0`.`publisher_id` = `e2`.`id` ' + 'where (((`e1`.`name` in (?, ?) or `e1`.`email` like ?)) or `e2`.`name` = ?)'
    );
    ...
    expect(qb4.getQuery()).toEqual(
      'select `e0`.*, `e0`.price * 1.19 as `price_taxed` from `book2` as `e0` ' + 'left join `author2` as `e1` on `e0`.`author_id` = `e1`.`id` ' + 'left join `publisher2` as `e2` on `e0`.`publisher_id` = `e2`.`id` ' + 'where (((not (`e1`.`name` = ?) or `e1`.`email` like ?)) or `e2`.`name` = ?)'
    );
    ...
    expect(qb5.getQuery()).toEqual('select `e0`.* from `author2` as `e0` ' + 'left join `book2` as `e1` on `e0`.`id` = `e1`.`author_id` ' + 'left join `publisher2` as `e2` on `e1`.`publisher_id` = `e2`.`id` ' + 'where (`e1`.`title` = ? or `e2`.`name` = ?)');
    ...
    expect(qb6.getQuery()).toEqual('select `e0`.* from `author2` as `e0` ' + 'left join `book2` as `e1` on `e0`.`id` = `e1`.`author_id` ' + 'left join `publisher2` as `e2` on `e1`.`publisher_id` = `e2`.`id` ' + 'where (`e2`.`name` = ? or `e2`.`name` = ?)');
    ...
    expect(qb7.getQuery()).toEqual('select `e0`.*, `e0`.price * 1.19 as `price_taxed` ' + 'from `book2` as `e0` ' + 'left join `book2_tags` as `e2` on `e0`.`uuid_pk` = `e2`.`book2_uuid_pk` ' + 'left join `book_tag2` as `e1` on `e2`.`book_tag2_id` = `e1`.`id` ' + 'where `e1`.`name` in (?, ?)');
    expect(qb7.getParams()).toEqual(['Tag 1', 'Tag 2']);

    const qb8 = orm.em.createQueryBuilder(BookTag2);
    qb8.select('*').where({ books: { test: { name: { $in: ['Test 1', 'Test 2'] } } } });
    expect(qb8.getQuery()).toEqual(
      'select `e0`.* ' + 'from `book_tag2` as `e0` ' + 'left join `book2_tags` as `e2` on `e0`.`id` = `e2`.`book_tag2_id` ' + 'left join `book2` as `e1` on `e2`.`book2_uuid_pk` = `e1`.`uuid_pk` ' + 'left join `test2` as `e3` on `e1`.`uuid_pk` = `e3`.`book_uuid_pk` ' + 'where `e3`.`name` in (?, ?)'
    );
  ...
  test('qb.raw() with named bindings', async () => {
    const qb = orm.em.createQueryBuilder(Book2);
    const raw = qb.raw<any>(`jsonb_set(payload, '$.{consumed}', :val)`, {
      val: 123,
    });
    qb.update({ meta: raw }).where({ uuid: '456' });
    expect(qb.getFormattedQuery()).toEqual("update `book2` set `meta` = jsonb_set(payload, '$.{consumed}', 123) where `uuid_pk` = '456'");
  ...
  test('update query with auto-joining', async () => {
    ...
    expect(qb.getQuery()).toEqual('update `publisher2` set `name` = ?, `type` = ? ' + 'where `id` in (select `e0`.`id` from (' + 'select distinct `e0`.`id` from `publisher2` as `e0` left join `book2` as `e1` on `e0`.`id` = `e1`.`publisher_id` where `e1`.`author_id` = ?' + ') as `e0`)');
  ...
  test('update query with or condition and auto-joining', async () => {
    ...
    expect(qb.getQuery()).toEqual(
      'update `publisher2` set `name` = ?, `type` = ? ' + 'where `id` in (select `e0`.`id` from (' + 'select distinct `e0`.`id` from `publisher2` as `e0` left join `book2` as `e1` on `e0`.`id` = `e1`.`publisher_id` where (`e1`.`author_id` = ? or `e1`.`title` = ?)' + ') as `e0`)'
    );
  ...
  test('delete query with auto-joining', async () => {
    ...
    expect(qb.getQuery()).toEqual('delete from `publisher2` where `id` in (select `e0`.`id` from (' + 'select distinct `e0`.`id` from `publisher2` as `e0` left join `book2` as `e1` on `e0`.`id` = `e1`.`publisher_id` where `e1`.`author_id` = ?' + ') as `e0`)');
    expect(qb.getParams()).toEqual([123]);
  });
  ...
  test('delete query with composite keys and auto-joining', async () => {
    ...
    expect(qb.getQuery()).toEqual(
      'delete from `foo_param2` where (`bar_id`, `baz_id`) in (select `e0`.`bar_id`, `e0`.`baz_id` from (' + 'select distinct `e0`.`bar_id`, `e0`.`baz_id` from `foo_param2` as `e0` left join `foo_bar2` as `e1` on `e0`.`bar_id` = `e1`.`id` where `e1`.`baz_id` = ?' + ') as `e0`)'
    );
    expect(qb.getParams()).toEqual([123]);
  });
  ...
  test('delete query with or condition and auto-joining', async () => {
    ...
    expect(qb.getQuery()).toEqual('delete from `publisher2` where `id` in (select `e0`.`id` from (' + 'select distinct `e0`.`id` from `publisher2` as `e0` left join `book2` as `e1` on `e0`.`id` = `e1`.`publisher_id` where (`e1`.`author_id` = ? or `e1`.`title` = ?)' + ') as `e0`)');
  ...
  test('clone QB', async () => {
    const qb = orm.em.createQueryBuilder(Publisher2, 'p').select(['p.*', 'b.*', 'a.*', 't.*']).leftJoin('books', 'b').join('b.author', 'a').join('b.tags', 't').where({ 'p.name': 'test 123', 'b.title': /3$/ }).orderBy({ 'b.title': QueryOrder.DESC });
  ...
  test('automatic pagination when to-many join detected together with offset/limit', async () => {
    const qb = orm.em.createQueryBuilder(Publisher2, 'p').leftJoinAndSelect('books', 'b').joinAndSelect('b.author', 'a').joinAndSelect('b.tags', 't').where({ 'p.name': 'test 123', 'b.title': /3$/ }).orderBy({ 'b.title': QueryOrder.DESC }).limit(10, 5);
  ...
  test('disabling automatic pagination', async () => {
    const qb = orm.em.createQueryBuilder(Publisher2, 'p').select(['p.*', 'b.*', 'a.*', 't.*']).leftJoin('books', 'b').join('b.author', 'a').join('b.tags', 't').where({ 'p.name': 'test 123', 'b.title': /3$/ }).orderBy({ 'b.title': QueryOrder.DESC }).setFlag(QueryFlag.DISABLE_PAGINATE).limit(10, 5);
  ...
  test('qb.getCount() removes limit, offset and order by clauses', async () => {
    ...
    await orm.em.createQueryBuilder(Publisher2, 'p').select(['p.*', 'b.*', 'a.*', 't.*']).leftJoin('books', 'b').join('b.author', 'a').join('b.tags', 't').where({ 'p.name': 'test 123', 'b.title': /3$/ }).orderBy({ 'b.title': QueryOrder.DESC }).limit(10, 5).getCount('id', true);
  ...
  test('$or and $and combined', async () => {
    ...
    expect(qb8.getQuery()).toEqual('select `a`.* from `author2` as `a` where ' + '`a`.`name` = ? and `a`.`email` = ? and ' + '`a`.`name` = ? and `a`.`email` = ? and ' + '`a`.`name` = ? and `a`.`email` = ?');

    const qb9 = orm.em.createQueryBuilder(Author2, 'a');
    qb9.select('*').where({
      $or: [{ $and: [{ name: 'value1', email: 'value2' }] }, { $not: { name: 'value3', email: 'value4' } }, { $or: [{ name: 'value5', email: 'value6' }] }, { $and: [{ name: 'value7', email: 'value8' }] }],
    });
    expect(qb9.getQuery()).toEqual('select `a`.* from `author2` as `a` where ' + '((`a`.`name` = ? and `a`.`email` = ?) or ' + 'not (`a`.`name` = ? and `a`.`email` = ?) or ' + '(`a`.`name` = ? and `a`.`email` = ?) or ' + '(`a`.`name` = ? and `a`.`email` = ?))');
  ...
  test('select and order by auto-joined property', async () => {
    ...
    expect(qb1.getQuery()).toEqual(
      'select `a`.*, `a`.price * 1.19 as `price_taxed` from `book2` as `a` ' + 'left join `author2` as `e1` on `a`.`author_id` = `e1`.`id` ' + 'left join `publisher2` as `e2` on `a`.`publisher_id` = `e2`.`id` ' + 'where (`e1`.`name` = ? or `e2`.`name` = ?) ' + 'order by `e1`.`email` asc'
    );
  });
  ...
  test('select with auto-joining and $not (GH issue #1537)', async () => {
    ...
    expect(qb1.getQuery()).toEqual('select `a`.*, `a`.price * 1.19 as `price_taxed` ' + 'from `book2` as `a` ' + 'left join `author2` as `e1` on `a`.`author_id` = `e1`.`id` ' + 'where (`e1`.`name` = ? or not (`e1`.`name` = ?))');
  });

  test('select with auto-joining and alias replacement via expr()', async () => {
    const qb1 = orm.em.createQueryBuilder(Book2, 'a');
    qb1.select('*').where({
      $or: [{ author: { name: 'test' } }, { author: { [expr((a) => `lower(${a}.name)`)]: 'wut' } }],
    });
    expect(qb1.getQuery()).toEqual('select `a`.*, `a`.price * 1.19 as `price_taxed` ' + 'from `book2` as `a` ' + 'left join `author2` as `e1` on `a`.`author_id` = `e1`.`id` ' + 'where (`e1`.`name` = ? or lower(e1.name) = ?)');
  });
  ...
  test('pivot joining of m:n when target entity is null (GH issue 548)', async () => {
    ...
    expect(qb11.getQuery()).toMatch('select `u`.* ' + 'from `user2` as `u` ' + 'left join `user2_cars` as `e1` on `u`.`first_name` = `e1`.`user2_first_name` and `u`.`last_name` = `e1`.`user2_last_name` ' + 'where (`e1`.`car2_name`, `e1`.`car2_year`) is null');
    ...
    expect(qb2.getQuery()).toMatch('select `b`.*, `b`.price * 1.19 as `price_taxed` ' + 'from `book2` as `b` ' + 'left join `book2_tags` as `e1` on `b`.`uuid_pk` = `e1`.`book2_uuid_pk` ' + 'where (`e1`.`book_tag2_id` is null or `e1`.`book_tag2_id` != ?)');
    expect(qb2.getParams()).toEqual(['1']);
    ...
    expect(qb3.getQuery()).toMatch('select `a`.* ' + 'from `author2` as `a` ' + 'left join `author_to_friend` as `e1` on `a`.`id` = `e1`.`author2_1_id` ' + 'left join `author2` as `e2` on `e1`.`author2_2_id` = `e2`.`id` ' + 'where `e1`.`author2_2_id` is null ' + 'order by `e2`.`name` asc');
    ...
    expect(qb4.getQuery()).toMatch('select `a`.* ' + 'from `author2` as `a` ' + 'left join `author_to_friend` as `e1` on `a`.`id` = `e1`.`author2_1_id` ' + 'where `e1`.`author2_2_id` is null ' + 'order by `e1`.`author2_2_id` asc');
    expect(qb4.getParams()).toEqual([]);
  });
  ...
  test('pivot joining of m:n when no target entity needed directly (GH issue 549)', async () => {
    const qb1 = orm.em
      ...
      .select('b.*')
      .where({ tags: { id: 1 } });
    expect(qb1.getQuery()).toMatch('select `b`.*, `b`.price * 1.19 as `price_taxed` ' + 'from `book2` as `b` ' + 'left join `book2_tags` as `e1` on `b`.`uuid_pk` = `e1`.`book2_uuid_pk` ' + 'where `e1`.`book_tag2_id` = ?');
    expect(qb1.getParams()).toEqual(['1']);
    ...
    expect(qb11.getQuery()).toMatch('select `u`.* ' + 'from `user2` as `u` ' + 'left join `user2_cars` as `e1` on `u`.`first_name` = `e1`.`user2_first_name` and `u`.`last_name` = `e1`.`user2_last_name` ' + 'where (`e1`.`car2_name`, `e1`.`car2_year`) = (?, ?)');
    ...
    expect(qb12.getQuery()).toMatch('select `u`.* ' + 'from `user2` as `u` ' + 'left join `user2_cars` as `e1` on `u`.`first_name` = `e1`.`user2_first_name` and `u`.`last_name` = `e1`.`user2_last_name` ' + 'where (`e1`.`car2_name`, `e1`.`car2_year`) in ((?, ?), (?, ?))');
    ...
    expect(qb4.getQuery()).toMatch('select `a`.* ' + 'from `author2` as `a` ' + 'left join `author_to_friend` as `e1` on `a`.`id` = `e1`.`author2_1_id` ' + 'where `e1`.`author2_2_id` = ? ' + 'order by `e1`.`author2_2_id` asc');
    expect(qb4.getParams()).toEqual([1]);
  });
  ...
  test('complex condition for json property with update query (GH #2839)', async () => {
    ...
    expect(qb141.getFormattedQuery()).toBe('update `book2` set `meta` = \'{\\"items\\":3}\' ' + "where `uuid_pk` = 'b47f1cca-90ca-11ec-99e0-42010a5d800c' " + 'and (`meta` is null ' + 'or `meta` is null ' + "or `meta`->'$.time' < 1646147306)");
  });
  ...
  test('branching to-many relations (#2677)', async () => {
    ...
    expect(qb2.getQuery()).toEqual('select `e0`.*, `e0`.price * 1.19 as `price_taxed` ' + 'from `book2` as `e0` ' + 'left join `author2` as `e1` on `e0`.`author_id` = `e1`.`id` ' + 'where `e1`.`name` = ? and `e1`.`name` = ?');
    ...
    expect(qb3.getQuery()).toEqual('select `e0`.*, `e0`.price * 1.19 as `price_taxed` ' + 'from `book2` as `e0` ' + 'left join `author2` as `e1` on `e0`.`author_id` = `e1`.`id` ' + 'where (`e1`.`name` = ? or `e1`.`name` = ?)');
    ...
    expect(qb4.getQuery()).toEqual('select `e0`.* ' + 'from `author2` as `e0` ' + 'left join `book2` as `e1` on `e0`.`id` = `e1`.`author_id` ' + 'left join `book2` as `e2` on `e0`.`id` = `e2`.`author_id` ' + 'where `e1`.`title` = ? and `e2`.`title` = ?');
    ...
    expect(qb5.getQuery()).toEqual('select `e0`.* ' + 'from `author2` as `e0` ' + 'left join `book2` as `e1` on `e0`.`id` = `e1`.`author_id` ' + 'where (`e1`.`title` = ? or `e1`.`title` = ?)');
    ...
    expect(qb6.getQuery()).toEqual(
      'select `e0`.*, `e0`.price * 1.19 as `price_taxed` ' + 'from `book2` as `e0` ' + 'left join `book2_tags` as `e2` on `e0`.`uuid_pk` = `e2`.`book2_uuid_pk` ' + 'left join `book_tag2` as `e1` on `e2`.`book_tag2_id` = `e1`.`id` ' + 'where `e1`.`name` = ? and `e1`.`name` = ?'
    );
    ...
    expect(qb7.getQuery()).toEqual(
      'select `e0`.*, `e0`.price * 1.19 as `price_taxed` ' + 'from `book2` as `e0` ' + 'left join `author2` as `e1` on `e0`.`author_id` = `e1`.`id` ' + 'left join `book2` as `e2` on `e1`.`favourite_book_uuid_pk` = `e2`.`uuid_pk` ' + 'where `e2`.`title` = ? and `e2`.`title` = ?'
    );
  ...
  test('postgres', async () => {
    ...
    expect(qb1.getQuery()).toEqual('select "p0".* from "publisher2" as "p0" where "p0"."name" @> $1');
    ...
    expect(qb2.getQuery()).toEqual('select "p0".* from "publisher2" as "p0" where "p0"."name" <@ $1');
    expect(qb2.getParams()).toEqual(['test']);

    const qb3 = pg.em.createQueryBuilder(Publisher2);
    qb3.select('*').where({ name: { $overlap: 'test' } });
    expect(qb3.getQuery()).toEqual('select "p0".* from "publisher2" as "p0" where "p0"."name" && $1');
    expect(qb3.getParams()).toEqual(['test']);

    const qb4 = pg.em.createQueryBuilder(Publisher2);
    qb4.select('*').where({ name: { $ilike: 'test' } });
    expect(qb4.getQuery()).toEqual('select "p0".* from "publisher2" as "p0" where "p0"."name" ilike $1');
    ...
    expect(qb141.getFormattedQuery()).toBe('update "book2" set "meta" = \'{"items":3}\' ' + 'where "uuid_pk" = \'b47f1cca-90ca-11ec-99e0-42010a5d800c\' ' + 'and ("meta" is null ' + 'or "meta" is null ' + 'or ("meta"->>\'time\')::float8 < 1646147306)');
    ...
    await pg.em.transactional(async (em) => {
      ...
      expect(qb1.getQuery()).toEqual('select "b0".*, "b0".price * 1.19 as "price_taxed" from "book2" as "b0" where "b0"."title" = $1 for share skip locked');

      const qb2 = em.createQueryBuilder(Book2);
      qb2.select('*').where({ title: 'test 123' }).setLockMode(LockMode.PESSIMISTIC_PARTIAL_WRITE);
      expect(qb2.getQuery()).toEqual('select "b0".*, "b0".price * 1.19 as "price_taxed" from "book2" as "b0" where "b0"."title" = $1 for update skip locked');

      const qb3 = em.createQueryBuilder(Book2);
      qb3.select('*').where({ title: 'test 123' }).setLockMode(LockMode.PESSIMISTIC_READ_OR_FAIL);
      expect(qb3.getQuery()).toEqual('select "b0".*, "b0".price * 1.19 as "price_taxed" from "book2" as "b0" where "b0"."title" = $1 for share nowait');

      const qb4 = em.createQueryBuilder(Book2);
      qb4.select('*').where({ title: 'test 123' }).setLockMode(LockMode.PESSIMISTIC_WRITE_OR_FAIL);
      expect(qb4.getQuery()).toEqual('select "b0".*, "b0".price * 1.19 as "price_taxed" from "book2" as "b0" where "b0"."title" = $1 for update nowait');
      ...
      expect(qb5.getQuery()).toEqual('select "b0".*, "b0".price * 1.19 as "price_taxed" from "book2" as "b0" left join "author2" as "a" on "b0"."author_id" = "a"."id" where "b0"."title" = $1 for update of "book2"');
    });
  ...
  test('count query with auto-joining (GH issue 858)', async () => {
    ...
    const sql1 = orm.em
      ...
      .getQuery();
    expect(sql1).toBe('select count(*) as `count` ' + 'from `author2` as `e0` ' + 'left join `book2` as `e1` on `e0`.`favourite_book_uuid_pk` = `e1`.`uuid_pk` ' + 'left join `test2` as `e2` on `e1`.`uuid_pk` = `e2`.`book_uuid_pk` ' + 'where `e2`.`id` = ?');
    ...
    const sql2 = orm.em
      ...
      .where({ favouriteBook: { test: { id: 1 } } })
      .getQuery();
    expect(sql2).toBe('select `e0`.* ' + 'from `author2` as `e0` ' + 'left join `book2` as `e1` on `e0`.`favourite_book_uuid_pk` = `e1`.`uuid_pk` ' + 'left join `test2` as `e2` on `e1`.`uuid_pk` = `e2`.`book_uuid_pk` ' + 'where `e2`.`id` = ?');
    ...
    const sql3 = orm.em
      ...
      .where({ test: { id: 1 } })
      .getQuery();
    expect(sql3).toBe('select count(*) as `count` ' + 'from `book2` as `e0` ' + 'left join `test2` as `e1` on `e0`.`uuid_pk` = `e1`.`book_uuid_pk` ' + 'where `e1`.`id` = ?');
    ...
    const sql4 = orm.em
      ...
      .where({ test: { id: 1 } })
      .getQuery();
    expect(sql4).toBe('select `e0`.*, `e1`.`id` as `test_id`, `e0`.price * 1.19 as `price_taxed` ' + 'from `book2` as `e0` ' + 'left join `test2` as `e1` on `e0`.`uuid_pk` = `e1`.`book_uuid_pk` ' + 'where `e1`.`id` = ?');
  });
