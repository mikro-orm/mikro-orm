...
describe('TPT (Table-Per-Type) Inheritance [mssql]', () => {
  ...
  beforeAll(async () => {
    ...
    // Create schema manually — the schema generator doesn't yet handle TPT table ordering on MSSQL
    await orm.schema.execute('create table [tpt_base] ([id] int identity(1,1) not null primary key, [base_name] nvarchar(255) not null, [created_at] datetime2 not null constraint [tpt_base_created_at_default] default getdate())');
    await orm.schema.execute('create table [tpt_child] ([id] int not null primary key, [child_value] nvarchar(255) not null)');
    await orm.schema.execute('alter table [tpt_child] add constraint [tpt_child_id_foreign] foreign key ([id]) references [tpt_base] ([id]) on update cascade on delete cascade');
  }, 120_000);
