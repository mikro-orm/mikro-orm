...
describe('QueryBuilder - Advanced', () => {
  ...
  test('clone QB', async () => {
    ...
    expect(clone.state.aliasCounter).toBe(qb.state.aliasCounter);
    expect(clone.state.flags).not.toBe(qb.state.flags);
    expect(clone.state.finalized).toBe(qb.state.finalized);
    expect(clone.state.fields).not.toBe(qb.state.fields);
    expect(clone.state.populate).not.toBe(qb.state.populate);
    expect(clone.state.populateMap).not.toBe(qb.state.populateMap);
    expect(clone.state.joins).not.toBe(qb.state.joins);
    expect(clone.state.aliases).not.toBe(qb.state.aliases);
    expect(clone.state.cond).not.toBe(qb.state.cond);
    expect(clone.state.orderBy).not.toBe(qb.state.orderBy);
    expect(clone.state.limit).toBe(qb.state.limit);
    expect(clone.state.offset).toBe(qb.state.offset);
