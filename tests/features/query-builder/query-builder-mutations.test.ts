...
describe('QueryBuilder - Mutations', () => {
  ...
  test('collation', async () => {
    ...
    const sql2 = orm.em
      ...
      .getFormattedQuery();
    expect(sql2).toBe(
      'select `e0`.* from `author2` as `e0` order by `e0`.`name` collate `utf8_general_ci` asc, `e0`.`email` collate `utf8_general_ci` desc',
    );

    // without collation, normal behavior
    const sql3 = orm.em.createQueryBuilder(Author2).orderBy({ name: 'asc' }).getFormattedQuery();
