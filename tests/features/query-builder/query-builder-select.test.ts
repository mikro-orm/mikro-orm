...
describe('QueryBuilder - Select', () => {
  ...
  test('select with 1:1 owner auto-join', async () => {
    ...
    const sql =
      'select `fz`.*, `e1`.`id` as `e1__id` from `foo_baz2` as `fz` left join `foo_bar2` as `e1` on `fz`.`id` = `e1`.`baz_id` limit ? offset ?';
    expect(qb.getQuery()).toEqual(sql);
    expect(qb.getParams()).toEqual([2, 1]);
  });
  ...
  test('select with custom expression', async () => {
    ...
    expect(qb2.getQuery()).toEqual(
      'select `e0`.*, `e0`.`price` * 1.19 as `price_taxed` from `book2` as `e0` where json_contains(`e0`.??, ?) = ?',
    );
  ...
  test('select formula property with string alias', async () => {
    ...
    expect(qb1.getQuery()).toEqual(
      'select `e0`.`id`, (select 123) as `rnd` from `foo_bar2` as `e0` where `e0`.`id` = ?',
    );
    expect(qb1.getParams()).toEqual([1]);
    ...
    expect(qb2.getQuery()).toEqual(
      'select `e0`.`id`, `e0`.`name` as `userName` from `foo_bar2` as `e0` where `e0`.`id` = ?',
    );
    expect(qb2.getParams()).toEqual([1]);
    ...
    expect(qb4.getQuery()).toEqual(
      'select `fb`.`id`, (select 123) as `rnd` from `foo_bar2` as `fb` where `fb`.`id` = ?',
    );
    ...
    expect(qb5.getQuery()).toEqual(
      'select `b`.`title`, `b`.`price` * 1.19 as `tax` from `book2` as `b` where `b`.`title` = ?',
    );
  ...
  test('select regular property with sql.ref().as()', async () => {
    ...
    expect(qb2.getQuery()).toEqual(
      'select `fb`.`id`, `fb`.`name` as `userName` from `foo_bar2` as `fb` where `fb`.`id` = ?',
    );
    expect(qb2.getParams()).toEqual([1]);
  });
  ...
  test('select formula via sql.ref() without alias', async () => {
    ...
    expect(qb.getQuery()).toEqual(
      'select `e0`.`id`, (select 123) as `random` from `foo_bar2` as `e0` where `e0`.`id` = ?',
    );
    expect(qb.getParams()).toEqual([1]);
  });
  ...
  test('select with join context alias', async () => {
    ...
    expect(qb.getQuery()).toEqual(
      'select `a`.`id`, `b`.`title` as `bookTitle` from `author2` as `a` left join `book2` as `b` on `a`.`id` = `b`.`author_id`',
    );
    ...
    expect(qb2.getQuery()).toEqual(
      'select `a`.`id`, `b`.`price` * 1.19 as `tax` from `author2` as `a` left join `book2` as `b` on `a`.`id` = `b`.`author_id`',
    );
  });
  ...
  test('select embedded with alias', async () => {
    ...
    expect(qb.getQuery()).toEqual(
      'select `a`.`id`, `a`.`identity` as `ident` from `author2` as `a` where `a`.`id` = ?',
    );
    expect(qb.getParams()).toEqual([1]);
  });
  ...
  test('select alias feeds into orderBy()', async () => {
    ...
    const qb2 = orm.em.createQueryBuilder(FooBar2, 'fb').select(['fb.id', 'random as rnd']).orderBy({ rnd: 'asc' });
    expect(qb2.getQuery()).toEqual(
  ...
  test('select with populate and join of 1:m', async () => {
    ...
    qb.select('*')
      ...
      .leftJoin('books', 'b');
    expect(qb.getQuery()).toEqual(
      'select `e0`.* ' + 'from `author2` as `e0` ' + 'left join `book2` as `b` on `e0`.`id` = `b`.`author_id`',
    );
  });
