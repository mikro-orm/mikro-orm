...
describe('QueryBuilder - Postgres', () => {
  ...
  test('collation', async () => {
    const sql1 = pg.em.createQueryBuilder(Author2).collation('en_US').orderBy({ name: 'asc' }).getFormattedQuery();
    ...
    const sql2 = pg.em
      ...
      .getFormattedQuery();
    expect(sql2).toBe(
      'select "a0".* from "author2" as "a0" order by "a0"."name" collate "en_US" asc, "a0"."email" collate "en_US" desc',
    );

    // without collation, normal behavior
    const sql3 = pg.em.createQueryBuilder(Author2).orderBy({ name: 'asc' }).getFormattedQuery();
