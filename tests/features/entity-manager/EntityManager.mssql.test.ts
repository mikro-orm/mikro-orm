...
describe('EntityManagerMsSql', () => {
  ...
  test('driver appends errored query', async () => {
    ...
    await expect(driver.nativeInsert('not_existing' as any, { foo: 'bar' })).rejects.toThrow(err1);
    const err2 = `Invalid object name 'not_existing'.`;
    await expect(driver.nativeDelete('not_existing' as any, {})).rejects.toThrow(err2);
  });
  ...
  test('populate queries respect the root condition (query condition propagation)', async () => {
    ...
    expect(mock.mock.calls[0][0]).toMatch('select [a0].*, [f1].[uuid_pk] as [f1__uuid_pk] ' +
      'from [author2] as [a0] ' +
      ...
      'where [b2].[title] in (?, ?)');
    expect(mock.mock.calls[1][0]).toMatch('select [b0].*, ([b0].[price] * 1.19) as [price_taxed] ' +
      'from [book2] as [b0] ' +
  ...
  test('exceptions', async () => {
    ...
    await expect(driver.nativeInsert('not_existing' as any, { foo: 'bar' })).rejects.toThrow(TableNotFoundException);
