...
describe.each(['sqlite', 'libsql'] as const)('EntityManager (%s)', driver => {
  ...
  test('should return sqlite driver', async () => {
    ...
    expect(await driver.findOne<any>(Book4, { title: '123' })).toBeNull();
    ...
    expect(await driver.find<any>(BookTag4, { books: [1] })).not.toBeNull();
  ...
  test('driver appends errored query', async () => {
    const driver = orm.em.getDriver();
    const err1 = 'no such table: Test';
    class Test {}
    await expect(driver.nativeInsert(Test, { foo: 'bar' })).rejects.toThrow(err1);
    const err2 = 'no such table: Test';
    await expect(driver.nativeDelete(Test, {})).rejects.toThrow(err2);
  });
  ...
  test('exceptions', async () => {
    ...
    class Test {}
    await expect(driver.nativeInsert(Test, { foo: 'bar' })).rejects.toThrow(TableNotFoundException);
