...
Maybe you want to check, or regenerate your yarn.lock or package-lock.json file?`);
  ...
  test('getDriverDependencies', async () => {
    await expect(CLIHelper.getDriverDependencies(await ConfigurationLoader.getConfiguration(ConfigurationLoader.getConfigPaths(), undefined, false))).resolves.toEqual([]);
    pathExistsMock.mockImplementation(async path => {
      const str = path as string;
      return str.includes('/mikro-orm.config.js') && !str.includes('/src/mikro-orm.config.js');
    });
    await expect(CLIHelper.getDriverDependencies(await ConfigurationLoader.getConfiguration(ConfigurationLoader.getConfigPaths()))).resolves.toEqual(['mongodb']);
  });
  ...
  test('getConfigPathFromArgs', async () => {
    ...
    await MikroORM.init();
    expect(messages[0]).toBe('[info] Path for config file was inferred from the command line arguments. This is deprecated. Set the MIKRO_ORM_CLI_CONFIG environment variable to specify the path, or if you really must use the command line arguments, import the config manually based on them, and pass it to init.');
    messages.length = 0;
