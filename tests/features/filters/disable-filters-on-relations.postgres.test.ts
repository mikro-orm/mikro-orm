...
describe('disable filters on relations [postgres]', () => {
  ...
  test('merging $or conditions', async () => {
    ...
    expect(mock.mock.calls[1][0]).toMatch(`select "m0".*, "u1"."id" as "u1__id" from "membership" as "m0" inner join "user" as "u1" on "m0"."user_id" = "u1"."id" where ("u1"."first_name" = $1 or "u1"."last_name" = $2 or "u1"."age" = $3) and ("m0"."role" = $4 or "m0"."role" = $5)`);
