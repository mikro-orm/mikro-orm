...
describe('disable filters on relations [postgres]', () => {
  ...
  test('get one employee with benefit status = A', async () => {
    ...
    expect(mock.mock.calls[2][0]).toMatch(
      `select "b0".*, "b1"."id" as "b1__id" from "benefit_detail" as "b0" inner join "benefit" as "b1" on "b0"."benefit_id" = "b1"."id" where "b0"."active" = ? and "b0"."benefit_id" in (?)`,
    );
  ...
  test('merging $or conditions', async () => {
    ...
    expect(mock.mock.calls[1][0]).toMatch(
      `select "m0".*, "u1"."id" as "u1__id" from "membership" as "m0" inner join "user" as "u1" on "m0"."user_id" = "u1"."id" where ("u1"."first_name" = ? or "u1"."last_name" = ? or "u1"."age" = ?) and ("m0"."role" = ? or "m0"."role" = ?)`,
    );
    expect(mock.mock.calls[2][0]).toMatch(
  ...
  test('eager joined filter on relation', async () => {
    ...
    expect(mock.mock.calls[0][0]).toMatch(
      `select "b0".*, "b1"."id" as "b1__id" from "benefit_detail" as "b0" inner join "benefit" as "b1" on "b0"."benefit_id" = "b1"."id" where "b0"."active" = ?`,
    );
