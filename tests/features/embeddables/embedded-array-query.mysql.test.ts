...
describe.each(['mysql', 'mariadb'])('embedded array query [%s]', type => {
  let orm: MikroORM;
  ...
  test('querying embedded array properties', async () => {
    ...
    expect(mock.mock.calls[0][0]).toMatch(
      "select `u0`.* from `user` as `u0` where (select 1 from json_table(`u0`.`addresses`, '$[*]' columns (`city` text path '$.city')) as `__je0` where `__je0`.`city` = ? limit 1) is not null",
    );
    ...
    expect(mock.mock.calls[1][0]).toMatch(
      "select `u0`.* from `user` as `u0` where (select 1 from json_table(`u0`.`addresses`, '$[*]' columns (`number` double path '$.number')) as `__je0` where `__je0`.`number` > ? limit 1) is not null",
    );
