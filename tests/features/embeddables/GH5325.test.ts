...
test('5325', async () => {
  ...
  expect(mock.mock.calls).toEqual([
    ...
    'from `drug` as `d0` ' +
    "left join `drug_info` as `d1` on `d0`.`drug_info_id` = `d1`.`id` and `d1`.`entity_state` = 'Available' " +
    'left join `ingredient` as `i2` on json_extract(`d1`.`drug_info_ingredients`, \'$.ingredient_id\') = `i2`.`id` ' +
