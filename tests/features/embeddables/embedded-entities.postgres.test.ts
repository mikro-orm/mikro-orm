...
describe('embedded entities in postgresql', () => {
  ...
  test('query by complex custom expressions with JSON operator and casting (GH issue 1261)', async () => {
    ...
    expect(mock.mock.calls[0][0]).toMatch('select "u0".* ' + 'from "user" as "u0" ' + "where (address4->>'street')::text != '' and " + "lower((address4->>'city')::text) = 'prague' and " + "(address4->>'city')::text = 'Prague' and " + "(address4->>'postalCode')::text = '12000'");
  });
});
