...
describe('$elemMatch on JSON properties in mssql', () => {
  ...
  test('$elemMatch queries', async () => {
    ...
    expect(mock.mock.calls[0][0]).toMatch(
      `select [e0].* from [event] as [e0] where exists (select 1 from openjson([e0].[tags]) with ([name] nvarchar(max) '$.name') as [__je0] where [__je0].[name] = ?)`,
    );
    ...
    expect(mock.mock.calls[0][0]).toMatch(
      `select [e0].* from [event] as [e0] where exists (select 1 from openjson([e0].[tags]) with ([priority] float '$.priority') as [__je0] where [__je0].[priority] > ?)`,
    );
