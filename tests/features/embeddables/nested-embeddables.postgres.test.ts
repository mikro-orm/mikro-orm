...
describe('embedded entities in postgres', () => {
  ...
  test('partial loading 2', async () => {
    ...
    await orm.em
      ...
      .execute();
    expect(mock.mock.calls[0][0]).toMatch(
      'select "u0"."profile1_identity_email" from "user" as "u0" where "u0"."profile1_identity_email" = ?',
    );

    await orm.em
      .fork()
      .findOne(User, { profile1: { identity: { email: 'foo@bar.baz' } } }, { fields: ['profile1.identity.email'] });
    expect(mock.mock.calls[1][0]).toMatch(
      'select "u0"."id", "u0"."profile1_identity_email" from "user" as "u0" where "u0"."profile1_identity_email" = ? limit ?',
    );
    ...
    expect(mock.mock.calls[2][0]).toMatch(
      'select "u0"."profile1_username", "u0"."profile1_identity_email", "u0"."profile1_identity_meta_foo", "u0"."profile1_identity_meta_bar", "u0"."profile1_identity_links" from "user" as "u0" where "u0"."profile1_identity_email" = ?',
    );

    await orm.em.fork().findOne(User, { profile1: { identity: { email: 'foo@bar.baz' } } }, { fields: ['profile1'] });
    expect(mock.mock.calls[3][0]).toMatch(
      'select "u0"."id", "u0"."profile1_username", "u0"."profile1_identity_email", "u0"."profile1_identity_meta_foo", "u0"."profile1_identity_meta_bar", "u0"."profile1_identity_links" from "user" as "u0" where "u0"."profile1_identity_email" = ? limit ?',
    );
    ...
    expect(mock.mock.calls[0][0]).toMatch(
      `select "u0"."profile2" from "user" as "u0" where "u0"."profile2"->'identity'->>'email' = ?`,
    );

    await orm.em
      .fork()
      .findOne(User, { profile2: { identity: { email: 'foo@bar.baz' } } }, { fields: ['profile2.identity.email'] }); // object embedded prop does not support nested partial loading
    expect(mock.mock.calls[1][0]).toMatch(
      `select "u0"."id", "u0"."profile2" from "user" as "u0" where "u0"."profile2"->'identity'->>'email' = ? limit ?`,
    );
    ...
    await orm.em
      .fork()
      .qb(User)
      .select('profile1_identity_links' as any)
      .where({ profile1: { identity: { links: { url: 'foo@bar.baz' } } } })
      .execute();
    expect(mock.mock.calls[2][0]).toMatch(
      `select "u0"."profile1_identity_links" from "user" as "u0" where "u0"."profile1_identity_links"->>'url' = ?`,
    ...
    await orm.em
      ...
      .execute(); // object embedded prop does not support nested partial loading
    expect(mock.mock.calls[0][0]).toMatch(
      `select "u0"."profile2" from "user" as "u0" where "u0"."profile2"->'identity'->'links'->>'url' = ?`,
    );
    ...
    expect(mock.mock.calls[1][0]).toMatch(
      `select "u0"."id", "u0"."profile2" from "user" as "u0" where "u0"."profile2"->'identity'->'links'->>'url' = ? limit ?`,
    );
    ...
    await orm.em.upsert(User, {
      ...
      profile1: {
        ...
        identity: {
          email: 'e1',
          meta: { foo: 'f1', bar: 'b1' },
          links: [{ url: 'l1', meta: { foo: 'f1', bar: 'b1' }, metas: [{ foo: 'f2', bar: 'b2' }] }],
        },
      },
      profile2: {
        username: 'u2',
        identity: {
          email: 'e2',
          meta: { foo: 'f2', bar: 'b2' },
          links: [{ url: 'l2', meta: { foo: 'f1', bar: 'b1' }, metas: [{ foo: 'f3', bar: 'b3' }] }],
        },
      },
    } as any);
  ...
  test('em.insert with nested embedded arrays (GH #7233)', async () => {
    ...
    await orm.em.insert(User, {
      ...
      profile2: {
        ...
        identity: {
          email: 'e2',
          links: [{ url: 'l3', meta: { foo: 'f3', bar: 'b3' }, metas: [{ foo: 'f4', bar: 'b4' }] }],
        },
      },
    });
    ...
    expect(insertQuery).toMatch(
      `'{"username":"u2","identity":{"email":"e2","links":[{"url":"l3","meta":{"foo":"f3","bar":"b3"},"metas":[{"foo":"f4","bar":"b4"}]}]}}'`,
    );
    // ensure metas is NOT double-encoded as a string within the JSON
