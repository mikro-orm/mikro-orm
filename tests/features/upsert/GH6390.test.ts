...
test('GH #6390', async () => {
  ...
  expect(mock.mock.calls[0][0]).toMatch(`merge into [TestEntityTable] using (values (N'2022-01-01', N'Test', N'2022-01-01', N'Test', N'Test'), (N'2022-01-01', N'Test', N'2022-01-01', N'Test', N'User 2')) as tsource([EndDate], [RevisionID], [StartDate], [Title], [UserID]) on [TestEntityTable].[RevisionID] = tsource.[RevisionID] and [TestEntityTable].[UserID] = tsource.[UserID] when not matched then insert ([EndDate], [RevisionID], [StartDate], [Title], [UserID]) values (tsource.[EndDate], tsource.[RevisionID], tsource.[StartDate], tsource.[Title], tsource.[UserID]) when matched then update set [EndDate]=tsource.[EndDate], [RevisionID]=tsource.[RevisionID], [StartDate]=tsource.[StartDate], [Title]=tsource.[Title], [UserID]=tsource.[UserID] output inserted.[DeliveryID];`);
