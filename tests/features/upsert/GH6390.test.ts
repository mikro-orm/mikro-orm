...
test('GH #6390', async () => {
  ...
  expect(mock.mock.calls[0][0]).toMatch(`merge into [TestEntityTable] using (values (N'2022-01-01', N'2022-01-01', N'Test', N'Test', N'Test'), (N'2022-01-01', N'2022-01-01', N'Test', N'User 2', N'Test')) as tsource([EndDate], [StartDate], [Title], [UserID], [RevisionID]) on [TestEntityTable].[RevisionID] = tsource.[RevisionID] and [TestEntityTable].[UserID] = tsource.[UserID] when not matched then insert ([EndDate], [StartDate], [Title], [UserID], [RevisionID]) values (tsource.[EndDate], tsource.[StartDate], tsource.[Title], tsource.[UserID], tsource.[RevisionID]) when matched then update set [EndDate] = tsource.[EndDate], [StartDate] = tsource.[StartDate], [Title] = tsource.[Title], [UserID] = tsource.[UserID], [RevisionID] = tsource.[RevisionID] output inserted.[DeliveryID];`);
