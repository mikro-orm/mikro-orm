...
describe('TimeType', () => {
  ...
  test('convertToDatabaseValue', () => {
    ...
    expect(() => type.convertToDatabaseValue(1, platform)).toThrow(`Could not convert JS value '1' of type 'number' to type TimeType`);
    expect(() => type.convertToDatabaseValue('2000-01-01', platform)).toThrow(`Could not convert JS value '2000-01-01' of type 'string' to type TimeType`);
    expect(() => type.convertToDatabaseValue(new Date('2000-01-01'), platform)).toThrow(`Could not convert JS value '2000-01-01T00:00:00.000Z' of type 'date' to type TimeType`);
  });
