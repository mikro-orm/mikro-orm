...
test('schema generator works with non-pk autoincrement columns (serial)', async () => {
  ...
  expect(mock.mock.calls[1][0]).toMatch(
    `'autoincrement' changed for column something._id { fromColumn: { name: '_id', type: 'int4', mappedType: IntegerType {}, length: null, precision: 32, scale: 0, nullable: false, default: null, unsigned: true, autoincrement: true, comment: null, primary: false, unique: false, enumItems: [] }, toColumn: { name: '_id', type: 'int', mappedType: IntegerType {}, unsigned: false, autoincrement: false, primary: false, nullable: false }}`,
  );
  expect(mock.mock.calls[2][0]).toMatch(
    `column something._id changed { changedProperties: Set(1) { 'autoincrement' } }`,
  );
  expect(mock.mock.calls[3][0]).toMatch(
    `'autoincrement' changed for column something._id { fromColumn: { name: '_id', type: 'int4', mappedType: IntegerType {}, length: null, precision: 32, scale: 0, nullable: false, default: null, unsigned: false, autoincrement: false, comment: null, primary: false, unique: false, enumItems: [] }, toColumn: { name: '_id', type: 'serial', mappedType: IntegerType {}, unsigned: true, autoincrement: true, primary: false, nullable: false }}`,
  );
