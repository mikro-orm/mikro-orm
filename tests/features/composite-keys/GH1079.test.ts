...
describe('GH issue 1079', () => {
  ...
  test(`GH issue 1079`, async () => {
    ...
    expect(queries[3]).toMatch(
      `insert into "deposit" ("tx_ref", "wallet_currency_ref", "wallet_owner__id", "amount", "gateway_key", "created_at", "updated_at", "status") values (?, ?, ?, ?, ?, ?, ?, ?)`,
    );
    expect(queries[4]).toMatch(`commit`);
    expect(queries[5]).toMatch(
      `select "w0".* from "wallet" as "w0" where "w0"."currency_ref" = ? and "w0"."owner__id" = ? limit ?`,
    );
