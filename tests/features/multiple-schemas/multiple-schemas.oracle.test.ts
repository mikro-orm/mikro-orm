import { BaseEntity, Cascade, Collection, LockMode, MikroORM, wrap } from '@mikro-orm/oracledb';
import {
  Entity,
  ManyToMany,
  ManyToOne,
...
describe('multiple connected schemas in oracle', () => {
  ...
  beforeEach(async () => {
    ...
    await orm.schema.clear({ schema: 'n3' });
    await orm.schema.clear({ schema: 'n4' });
    await orm.schema.clear({ schema: 'n5' });
  ...
  test('use different schema via options', async () => {
    ...
    await orm.em.persist(author).flush();
    expect(mock.mock.calls[0][0]).toMatch(`begin`);
    // Oracle uses RETURNING clause for inserts with identity columns
    expect(mock.mock.calls[1][0]).toMatch(`"n3"."book_tag"`);
    expect(mock.mock.calls[2][0]).toMatch(`"n5"."book_tag"`);
    expect(mock.mock.calls[3][0]).toMatch(`"n4"."book_tag"`);
    expect(mock.mock.calls[4][0]).toMatch(`"n1"."author"`);
    expect(mock.mock.calls[5][0]).toMatch(`"n3"."book"`);
    expect(mock.mock.calls[6][0]).toMatch(`"n5"."book"`);
    expect(mock.mock.calls[7][0]).toMatch(`"n4"."book"`);
    mock.mockReset();
    ...
    expect(mock.mock.calls[0][0]).toMatch(`begin`);
    expect(mock.mock.calls[1][0]).toMatch(`update "n3"."book_tag"`);
    expect(mock.mock.calls[2][0]).toMatch(`update "n4"."book_tag"`);
    expect(mock.mock.calls[3][0]).toMatch(`update "n5"."book_tag"`);
    expect(mock.mock.calls[4][0]).toMatch(`update "n1"."author"`);
    expect(mock.mock.calls[5][0]).toMatch(`update "n3"."book"`);
    expect(mock.mock.calls[6][0]).toMatch(`update "n4"."book"`);
    expect(mock.mock.calls[7][0]).toMatch(`update "n5"."book"`);
    mock.mockReset();

    const fork = orm.em.fork();
    await fork.findOneOrFail(Author, author, { populate: ['*'], schema: 'n5' });

    expect(mock.mock.calls[0][0]).toMatch(`select "a0".* from "n1"."author" "a0" where "a0"."id" = 1`);
    expect(mock.mock.calls[1][0]).toMatch(`select "b0".* from "n5"."book" "b0" where "b0"."author_id" in (1)`);
    expect(mock.mock.calls[2][0]).toMatch(`"n5"."book_tags"`);
    mock.mockReset();
  ...
  test('use different schema via options in em.insert/Many', async () => {
    ...
    expect(mock.mock.calls[0][0]).toMatch(`"n1"."author"`);
    expect(mock.mock.calls[1][0]).toMatch(`"n5"."book"`);
    expect(mock.mock.calls[2][0]).toMatch(`"n3"."book"`);
    expect(mock.mock.calls[3][0]).toMatch(`"n4"."book"`);
    mock.mockReset();
