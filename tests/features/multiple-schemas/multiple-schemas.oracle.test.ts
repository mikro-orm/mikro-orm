import { BaseEntity, Cascade, Collection, MikroORM, wrap } from '@mikro-orm/oracledb';
import {
  Entity,
  ManyToMany,
  ManyToOne,
...
describe('multiple connected schemas in oracle', () => {
  ...
  beforeAll(async () => {
    ...
    // Drop all tables across schemas in a single operation (avoids separate introspections per schema)
    await orm.em.execute(
      `begin for rec in (select owner, table_name from all_tables where owner in ('n1', 'n2', 'n3', 'n4', 'n5')) loop execute immediate 'drop table "' || rec.owner || '"."' || rec.table_name || '" cascade constraints'; end loop; end;`,
    );
  ...
  test('use different schema via options', async () => {
    ...
    expect(mock.mock.calls[2][0]).toMatch(
      `select "b0"."book_tag_id", "b0"."book_id", "b1"."id" "b1__id", "b1"."name" "b1__name" from "n5"."book_tags" "b0" inner join "n5"."book_tag" "b1" on "b0"."book_tag_id" = "b1"."id" where "b0"."book_id" in (1, 2)`,
    );
    mock.mockReset();
