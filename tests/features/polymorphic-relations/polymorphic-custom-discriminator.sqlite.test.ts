...
describe('polymorphic relations with custom discriminator map', () => {
  ...
  test('discriminator map prevents using table names directly', async () => {
    ...
    // When we try to load, it should not find the entity
    // because 'blog_post' is not in the discriminator map
    const activity = await orm.em.findOne(Activity, { id: 1 });

    // The activity should exist but subject might not hydrate properly
    // or might be null depending on implementation
    expect(activity).toBeDefined();
    // This behavior depends on how strictly the discriminator map is enforced
  });
