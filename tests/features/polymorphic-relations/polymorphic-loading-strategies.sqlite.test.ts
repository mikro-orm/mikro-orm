...
class Reaction {
  ...
  @ManyToOne(() => [Article, Video], { discriminator: 'target' })
...
describe('polymorphic relations with Ref wrapper', () => {
  ...
  class Comment {
    ...
    @ManyToOne(() => [Post, Image], { discriminator: 'target', ref: true })
...
describe('lazy loading polymorphic relations', () => {
  ...
  test('lazy loads Ref-wrapped polymorphic relation via ref.load()', async () => {
    ...
    class LazyComment {
      ...
      @ManyToOne(() => [LazyPost, LazyImage], { discriminator: 'target', ref: true })
