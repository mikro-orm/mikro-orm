...
describe('EntityGenerator', () => {
  ...
  test('generate entities from schema [mysql]', async () => {
    ...
    const dump = await orm.entityGenerator.generate({ save: true, path: './temp/EntityGeneratorMySQL/entities' });
    expect(dump).toMatchSnapshot('mysql-entity-dump');

    await expect(pathExists('./temp/EntityGeneratorMySQL/entities/Author2.ts')).resolves.toBe(true);
    await remove('./temp/EntityGeneratorMySQL/entities');
  ...
  test('skipTables [mysql]', async () => {
    ...
    const dump = await orm.entityGenerator.generate({
      save: true,
      path: './temp/EntityGeneratorMySQL/entities',
  ...
  test('generate EntitySchema with bidirectional relations and reference wrappers [mysql]', async () => {
    ...
    const orm2 = await MikroORM.init({
      driver: SqliteDriver,
      entities: ['./temp/EntityGeneratorMySQL/entities'],
      dbName: ':memory:',
    });
  ...
  test('generate entities with reference wrappers and named import [mysql]', async () => {
    ...
    const dump = await generator.generate({ save: true, path: './temp/EntityGeneratorMySQL/entities' });
  ...
  test('table name with underscore using entitySchema [mysql]', async () => {
    ...
    `);
    const dump = await orm.entityGenerator.generate({ save: false, path: './temp/EntityGeneratorMySQL/entities' });
