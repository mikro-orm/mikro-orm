import { type LoggerNamespace, MikroORM, ReferenceKind, wrap } from '@mikro-orm/sqlite';
...
class User {
  ...
  @Embedded(() => Address1, { prefix: false })
  address3 = new Address1();

  @Embedded(() => Address1, { object: true })
  address4 = new Address1();

  @Embedded(() => Address1, { object: true, nullable: true })
  address5?: Address1;
...
describe('embedded entities in mysql', () => {
  ...
  test('metadata', async () => {
    ...
    expect(orm.getMetadata().get<any>(User).properties.address1_street).toMatchObject({
    ...
    expect(orm.getMetadata().get<any>(User).properties.address2_street).toMatchObject({
    ...
    expect(orm.getMetadata().get<any>(User).properties['address4~street']).toMatchObject({
