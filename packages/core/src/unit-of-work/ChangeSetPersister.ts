...
export class ChangeSetPersister {
   ...
   */
  mapReturnedValues<T extends object>(entity: T, payload: EntityDictionary<T>, row: Dictionary | undefined, meta: EntityMetadata<T>, overrideProps?: (keyof T)[]): void {
    if (this.platform.usesReturningStatement() && row && Utils.hasObjectKeys(row)) {
      const data = meta.props.reduce((ret, prop) => {
        if (prop.fieldNames && (row[prop.fieldNames[0]] != null && entity[prop.name] == null || overrideProps?.includes(prop.name))) {
