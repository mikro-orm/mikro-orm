...
export class EntityLoader {
  ...
  async populate<Entity extends object, Fields extends string = PopulatePath.ALL>(
    ...
    populate = this.normalizePopulate<Entity>(
      entityName,
      populate as true,
      options.strategy as LoadStrategy | undefined,
      options.lookup,
  ...
  private async populateScalar<Entity extends object>(
  ...
  private async populatePolymorphic<Entity extends object>(
    entities: Entity[],
  ...
  private async findChildren<Entity extends object>(
    ...
    if (polymorphicOwnerProp && Array.isArray(fk)) {
      ...
      where = this.mergePrimaryCondition<Entity>(
        ids,
    ...
    if (prop.targetKey && [ReferenceKind.ONE_TO_ONE, ReferenceKind.MANY_TO_ONE].includes(prop.kind)) {
      ...
      for (const entity of entities) {
        ...
        if (loadedItem) {
          entity[prop.name] = (
            Reference.isReference(ref) ? Reference.create(loadedItem) : loadedItem
  ...
  private mergePrimaryCondition<Entity extends object>(
    ids: Entity[],
    pk: FilterKey<Entity>,
    ...
    const cond1 = QueryHelper.processWhere<Entity>({
      where: { [pk]: { $in: ids } } as FilterQuery<Entity>,
    ...
    return where[pk] ? ({ $and: [cond1, where] } as FilterQuery<any>) : { ...cond1, ...where };
  }
  ...
  /** @internal */
  async findChildrenFromPivotTable<Entity extends object>(
    filtered: Entity[],
    prop: EntityProperty<Entity>,
    options: Required<EntityLoaderOptions<Entity>>,
  ...
  private buildFields<Entity>(
    ...
    if (ref) {
      fields = prop.targetMeta!.primaryKeys.map(
        targetPkName => `${prop.name}.${targetPkName}`,
  ...
  private filterCollections<Entity extends object>(
    ...
    return entities.filter(
      e => Utils.isCollection(e[field]) && !(e[field] as unknown as Collection<AnyEntity>).isInitialized(!ref),
    );
  }
