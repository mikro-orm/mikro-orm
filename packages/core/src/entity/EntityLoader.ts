...
export class EntityLoader {
  ...
  async populate<Entity extends object, Fields extends string = PopulatePath.ALL>(entityName: string, entities: Entity[], populate: PopulateOptions<Entity>[] | boolean, options: EntityLoaderOptions<Entity, Fields>): Promise<void> {
    ...
    populate = this.normalizePopulate<Entity>(entityName, populate as true, options.strategy, options.lookup, options.exclude as string[]);
  ...
  normalizePopulate<Entity>(entityName: string, populate: (PopulateOptions<Entity> | boolean)[] | PopulateOptions<Entity> | boolean, strategy?: LoadStrategy, lookup = true, exclude?: string[]): PopulateOptions<Entity>[] {
    const meta = this.metadata.find(entityName)!;
  ...
  private lookupEagerLoadedRelationships<Entity>(entityName: string, populate: PopulateOptions<Entity>[], strategy?: LoadStrategy, prefix = '', visited: string[] = [], exclude?: string[]): PopulateOptions<Entity>[] {
    const meta = this.metadata.find<Entity>(entityName);
    ...
    meta.relations
      ...
      .forEach(prop => {
        ...
        const nested = this.lookupEagerLoadedRelationships<Entity>(prop.type, nestedPopulate as any, strategy, prefixed, visited.slice(), exclude);
