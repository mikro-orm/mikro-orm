...
export class ValidationError<T extends AnyEntity = AnyEntity> extends Error {
  ...
  static fromWrongPropertyType(
    ...
    const msg =
      `Trying to set ${entityName}.${property} of type '${expectedType}' to '${givenValue}' of type '${givenType}'`;
  ...
  static fromWrongRepositoryType(entityName: string, repoType: string, method: string): ValidationError {
    const msg =
      `Trying to use EntityRepository.${method}() with '${entityName}' entity while the repository is of type '${repoType}'`;
  ...
  static fromCollectionNotInitialized(entity: AnyEntity, prop: EntityProperty): ValidationError {
    const entityName = entity.constructor.name;
    const msg =
      `${entityName}.${prop.name} is not initialized, define it as '${prop.name} = new Collection<${prop.type}>(this);'`;
  ...
  static notEntity(owner: AnyEntity, prop: EntityProperty, data: any): ValidationError {
    ...
    return new ValidationError(
      `Entity of type ${prop.type} expected for property ${owner.constructor.name}.${prop.name}, ${
        inspect(data)
      } of type ${type} given. If you are using Object.assign(entity, data), use em.assign(entity, data) instead.`,
    );
  }
  ...
  static cannotUseOperatorsInsideEmbeddables(className: string, propName: string, payload: unknown): ValidationError {
    return new ValidationError(
      `Using operators inside embeddables is not allowed, move the operator above. (property: ${className}.${propName}, payload: ${
        inspect(payload)
      })`,
    );
  }
  ...
  static invalidEmbeddableQuery(className: string, propName: string, embeddableType: string): ValidationError {
    return new ValidationError(
      `Invalid query for entity '${className}', property '${propName}' does not exist in embeddable '${embeddableType}'`,
    );
  }
}
...
export class MetadataError<T extends AnyEntity = AnyEntity> extends ValidationError<T> {
  ...
  static fromWrongOwnership(
    ...
    return new MetadataError(
      `Both ${meta.className}.${prop.name} and ${prop.type}.${
        prop[key]
      } are defined as ${type} sides, use '${other}' on one of them`,
    );
  }
  ...
  static multipleVersionFields(meta: EntityMetadata, fields: string[]): MetadataError {
    return new MetadataError(
      `Entity ${meta.className} has multiple version properties defined: '${
        fields.join("', '")
      }'. Only one version property is allowed per entity.`,
    );
  }
  ...
  static fromUnknownEntity(className: string, source: string): MetadataError {
    return new MetadataError(
      `Entity '${className}' was not discovered, please make sure to provide it in 'entities' array when initializing the ORM (used in ${source})`,
    );
  }
  ...
  static conflictingPropertyName(className: string, name: string, embeddedName: string): MetadataError {
    return new MetadataError(
      `Property ${className}:${name} is being overwritten by its child property ${embeddedName}:${name}. Consider using a prefix to overcome this issue.`,
    );
  }
