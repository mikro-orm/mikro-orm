...
export class ValidationError<T extends AnyEntity = AnyEntity> extends Error {
  ...
  static invalidPropertyName(entityName: EntityName<any>, invalid: string): ValidationError {
  ...
  static cannotUseOperatorsInsideEmbeddables(entityName: EntityName<any>, propName: string, payload: unknown): ValidationError {
  ...
  static cannotUseGroupOperatorsInsideScalars(entityName: EntityName<any>, propName: string, payload: unknown): ValidationError {
  ...
  static invalidEmbeddableQuery(entityName: EntityName<any>, propName: string, embeddableType: string): ValidationError {
...
export class MetadataError<T extends AnyEntity = AnyEntity> extends ValidationError<T> {
  ...
  static fromUnknownEntity(entityName: EntityName<any>, source: string): MetadataError {
    return new MetadataError(`Entity '${Utils.className(entityName)}' was not discovered, please make sure to provide it in 'entities' array when initializing the ORM (used in ${source})`);
  }
  ...
  static duplicateFieldName(entityName: EntityName<any>, names: [string, string][]): MetadataError {
