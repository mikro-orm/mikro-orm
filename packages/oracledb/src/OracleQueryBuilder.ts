...
> extends QueryBuilder<Entity, RootAlias, Hint, Context> {
  ...
  override insert(data: RequiredEntityData<Entity> | RequiredEntityData<Entity>[]): InsertQueryBuilder<Entity> {
    for (const row of Utils.asArray(data)) {
      if (this.mainAlias.meta && Object.keys(row).length === 0) {
        ...
        row[this.mainAlias.meta.primaryKeys[0] as keyof typeof row] = raw('default');
      }
    }
  ...
  override getNativeQuery(processVirtualEntity = true): NativeQueryBuilder {
    ...
    const lockTables = (qb as any).options?.lockTables;
    if (lockTables?.length) {
      // console.log(lockTables, this.lockTables); // FIXME
  ...
  private convertLockTablesToColumnRefs(tables: string[]): void {
