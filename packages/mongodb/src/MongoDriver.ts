...
export class MongoDriver extends DatabaseDriver<MongoConnection> {
  ...
  override async aggregate(entityName: EntityName<any>, pipeline: any[], ctx?: Transaction<ClientSession>): Promise<any[]> {
  ...
  private buildFilterById<T extends { _id: any }>(entityName: EntityName<any>, id: string): FilterQuery<T> {
    const meta = this.metadata.find(entityName)!;
