...
> extends DatabaseDriver<Connection> {
        ...
        .map(row => {
          ...
          props.forEach(prop => {
            ...
            if (prop.fieldNames.length > 1) {
              ...
              if (prop.polymorphic && (row[prop.name] as unknown) instanceof PolymorphicRef) {
                const polyValue = row[prop.name] as unknown as PolymorphicRef;
  ...
  protected async loadPolymorphicPivotOwnerSide<T extends object, O extends object>(
    ...
    for (const item of res) {
      const ownerValue = (item as any)[prop.discriminator!];
      const key = Utils.getPrimaryKeyHash(Utils.asArray(ownerValue));
      map[key].push((item as any)[inverseProp!.name]);
    }
  ...
  protected async loadPolymorphicPivotInverseSide<T extends object, O extends object>(
    ...
    for (const item of res) {
      const tagValue = (item as any)[tagProp.name];
      const key = Utils.getPrimaryKeyHash(Utils.asArray(tagValue));
      const entity = (item as any)[ownerRelationName] as T;
      if (entity && map[key]) {
        map[key].push(entity);
      }
    }
