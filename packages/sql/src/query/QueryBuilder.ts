...
> {
   ...
   */
  // FIXME should we keep the custom type param or even remap the return type?
  from(target: QueryBuilder<any>, aliasName?: string): SelectQueryBuilder<Entity, RootAlias, Hint, Context>;
  from(target: EntityName<Entity>): SelectQueryBuilder<Entity, RootAlias, Hint, Context>;
  from(target: EntityName<Entity> | QueryBuilder<Entity>, aliasName?: string): SelectQueryBuilder<Entity, RootAlias, Hint, Context> {
    this.ensureNotFinalized();
   ...
   * @internal
   */
  getNextAlias(entityName: string | EntityName<any> = 'e'): string {
   ...
   * @internal
   */
  getAliasMap(): Dictionary<EntityName<any>> {
  ...
  as<T>(aliasOrTargetEntity: string | EntityName<T>, alias?: EntityKey<T>): NativeQueryBuilder {
    ...
    if (alias) {
      const meta = this.metadata.find(aliasOrTargetEntity as EntityName<T>);
      /* v8 ignore next */
  ...
  private joinReference(field: string | RawQueryFragment | NativeQueryBuilder | QueryBuilder, alias: string, cond: Dictionary, type: JoinType, path?: string, schema?: string, subquery?: string): { prop: EntityProperty<Entity>; key: string } {
    ...
    if (prop.kind === ReferenceKind.ONE_TO_MANY) {
      ...
      if (type !== JoinType.pivotJoin) {
        ...
        pivotAlias = oldPivotAlias ?? this.getNextAlias(Utils.className(prop.pivotEntity));
  ...
  private applyDiscriminatorCondition(): void {
    ...
    const types = Object.values(meta.root.discriminatorMap!).map(cls => this.metadata.getByClassName(cls)); // FIXME???
    const children: EntityMetadata[] = [];
    const lookUpChildren = (ret: EntityMetadata[], type: EntityName<any>) => {
